...
export function strengthenIdea(idea = {}, { feedback = [], priorArtRecords = [], projectPackage = null } = {}) {
  ...
  if (!has(out.processingLogic)) { out.processingLogic = 'Ingest → normalize → reliability-weight → fuse/score → calibrate → emit with confidence.'; changes.push('Added processing logic / algorithm steps.'); }
  if (!has(out.outputResult)) { out.outputResult = 'A calibrated, real-time decision/score with a confidence value and explanation.'; changes.push('Defined the output and confidence signal.'); }
  ...
  const after = scorePatentIdea(out, { priorArtRecords });
