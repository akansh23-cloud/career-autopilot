      ...
      <div className="border-t border-subtle p-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="h-11 flex-1 rounded-xl border border-field-border bg-field px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50"
          />
      ...
      <div className="space-y-3 border-b border-subtle p-4">
        <div className="relative">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search the help center…"
            className="h-11 w-full rounded-xl border border-field-border bg-field pl-10 pr-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
        </div>
      ...
      <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
        ...
        <div className="mt-2 flex items-center gap-2">
          <button onClick={() => setState({ status: 'idle', result: null, err: '' })}
            className="rounded-xl border border-subtle px-4 py-2 text-sm text-fg hover:bg-surface-1">Create another</button>
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <input value={form.name} onChange={set('name')} placeholder="Name"
            className="h-11 rounded-xl border border-field-border bg-field px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" />
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="relative">
            <select value={form.category} onChange={set('category')}
              className="h-11 w-full appearance-none rounded-xl border border-field-border bg-field px-3.5 pr-9 text-sm capitalize text-fg outline-none focus:border-aurora-violet/50">
              {CATEGORIES.map((c) => <option key={c} value={c} className="bg-menu capitalize">{c}</option>)}
            </select>
          ...
          <div className="relative">
            <select value={form.priority} onChange={set('priority')}
              className="h-11 w-full appearance-none rounded-xl border border-field-border bg-field px-3.5 pr-9 text-sm capitalize text-fg outline-none focus:border-aurora-violet/50">
              {['low', 'normal', 'high', 'urgent'].map((p) => <option key={p} value={p} className="bg-menu capitalize">{p}</option>)}
            </select>
            <ChevronDown size={15} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-fg-muted" />
          </div>
        </div>
        <input value={form.subject} onChange={set('subject')} placeholder="Subject *"
          className="h-11 w-full rounded-xl border border-field-border bg-field px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" />
        <textarea value={form.message} onChange={set('message')} placeholder="Describe the issue — steps, what you expected, what happened. *"
          className="h-32 w-full resize-none rounded-xl border border-field-border bg-field p-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" />
