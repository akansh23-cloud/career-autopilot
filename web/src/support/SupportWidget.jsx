    ...
    >
      <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${
        me
          ...
          : 'border border-subtle bg-surface-1 text-fg'
      }`}>
        {children}
      </div>
    </motion.div>
...
function Typing() {
  return (
    <div className="flex justify-start">
      <div className="flex items-center gap-1 rounded-2xl border border-subtle bg-surface-1 px-4 py-3">
        {[0, 1, 2].map((i) => (
  ...
  return (
    <button onClick={onClick}
      className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs transition ${
        active
          ? 'border-aurora-violet/40 bg-aurora-violet/15 text-fg'
          : 'border-subtle bg-surface-1 text-fg-secondary hover:border-strong hover:text-fg'
      }`}>
      {children}
    </button>
  );
            ...
            {m.from === 'bot' && m.suggestTicket && (
              <div className="pl-1">
                <button onClick={onTicket}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-aurora-violet/30 bg-aurora-violet/10 px-3 py-1.5 text-xs font-medium text-fg hover:bg-aurora-violet/20">
      ...
      <div className="border-t border-subtle p-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="h-11 flex-1 rounded-xl border border-subtle bg-surface-1 px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50"
          />
        ...
        <p className="mt-1.5 text-center text-[10px] text-fg-muted">Answers come from our help center — not guesses.</p>
      </div>
    </div>
  );
}
    ...
    <div className="flex h-full flex-col">
      <div className="space-y-3 border-b border-subtle p-4">
        <div className="relative">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search the help center…"
            className="h-11 w-full rounded-xl border border-subtle bg-surface-1 pl-10 pr-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
        </div>
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-4">
        ...
        {filtered.map((f) => {
          ...
          return (
            <div key={f.id} className="overflow-hidden rounded-xl border border-subtle bg-surface-1">
              <button onClick={() => setOpen(isOpen ? null : f.id)}
                className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm font-medium text-fg hover:bg-surface-1">
                {f.q}
                <ChevronDown size={16} className={`shrink-0 text-fg-muted transition ${isOpen ? 'rotate-180' : ''}`} />
              </button>
              <AnimatePresence>
                {isOpen && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }}>
                    <p className="whitespace-pre-line border-t border-subtle px-4 py-3 text-[13px] leading-relaxed text-fg-secondary">{f.a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
...
function TicketTab({ draft, clearDraft, authed, onClose }) {
  ...
  if (state.status === 'done') {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
        ...
        <h3 className="text-lg font-semibold text-fg">Ticket created</h3>
        <p className="max-w-xs text-sm text-fg-secondary">{state.result?.message || "We'll get back to you by email."}</p>
        {state.result?.ticket?.id && <p className="text-xs text-fg-muted">Ref: {state.result.ticket.id}</p>}
        {!state.result?.stored && (
        ...
        <div className="mt-2 flex items-center gap-2">
          <button onClick={() => setState({ status: 'idle', result: null, err: '' })}
            className="rounded-xl border border-white/12 px-4 py-2 text-sm text-fg hover:bg-surface-1">Create another</button>
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-4">
      <p className="mb-3 text-sm text-fg-secondary">Couldn’t find your answer? Send us the details and we’ll follow up.</p>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <input value={form.name} onChange={set('name')} placeholder="Name"
            className="h-11 rounded-xl border border-subtle bg-surface-1 px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" />
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="relative">
            <select value={form.category} onChange={set('category')}
              className="h-11 w-full appearance-none rounded-xl border border-subtle bg-surface-1 px-3.5 pr-9 text-sm capitalize text-fg outline-none focus:border-aurora-violet/50">
              {CATEGORIES.map((c) => <option key={c} value={c} className="bg-base capitalize">{c}</option>)}
            </select>
          ...
          <div className="relative">
            <select value={form.priority} onChange={set('priority')}
              className="h-11 w-full appearance-none rounded-xl border border-subtle bg-surface-1 px-3.5 pr-9 text-sm capitalize text-fg outline-none focus:border-aurora-violet/50">
              {['low', 'normal', 'high', 'urgent'].map((p) => <option key={p} value={p} className="bg-base capitalize">{p}</option>)}
            </select>
            <ChevronDown size={15} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-fg-muted" />
          </div>
        </div>
        <input value={form.subject} onChange={set('subject')} placeholder="Subject *"
          className="h-11 w-full rounded-xl border border-subtle bg-surface-1 px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" />
        <textarea value={form.message} onChange={set('message')} placeholder="Describe the issue — steps, what you expected, what happened. *"
          className="h-32 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" />
      ...
      {authed && (
        <div className="mt-6 border-t border-subtle pt-4">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-fg-muted">My tickets</h4>
          {!mine.loaded ? (
            <p className="text-xs text-fg-muted">Loading…</p>
          ) : mine.tickets.length === 0 ? (
            <p className="text-xs text-fg-muted">No tickets yet.</p>
          ) : (
            <div className="space-y-2">
              {mine.tickets.map((t) => (
                <div key={t.id} className="rounded-xl border border-subtle bg-surface-1 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <span className="truncate text-sm text-fg">{t.subject}</span>
                    <span className={`rounded-full px-2 py-0.5 text-[10px] capitalize ${
                      t.status === 'open' ? 'bg-aurora-cyan/15 text-aurora-cyan' :
                      t.status === 'resolved' ? 'bg-aurora-mint/15 text-aurora-mint' : 'bg-surface-2 text-fg-secondary'
                    }`}>{t.status}</span>
                  </div>
                  <p className="mt-0.5 text-[11px] capitalize text-fg-muted">{t.category} · {t.priority}</p>
                </div>
              ))}
            </div>
          )}
    ...
    <>
      ...
      {!open && (
        ...
        >
          ...
          <LifeBuoy size={24} className="text-fg transition group-hover:rotate-45" />
        </motion.button>
      )}
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[61] flex flex-col overflow-hidden border border-white/12 bg-base/85 backdrop-blur-2xl
          ...
          >
            ...
            <div className="relative shrink-0 overflow-hidden border-b border-subtle px-4 pb-3 pt-4">
              ...
              {/* tabs */}
              <div className="relative mt-3 flex gap-1 rounded-xl bg-surface-1 p-1">
                {TABS.map((t) => (
