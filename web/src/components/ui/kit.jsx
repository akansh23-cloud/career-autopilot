...
export const Button = forwardRef(function Button(
  ...
  const variants = {
    ...
    outline: 'border border-white/14 text-fg hover:bg-surface-1 hover:border-aurora-violet/40',
...
export function Badge({ tone = 'default', className, children }) {
  const tones = {
    default: 'bg-white/6 text-fg-secondary border-subtle',
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cx(
              'absolute z-50 mt-2 min-w-[220px] overflow-hidden rounded-2xl border border-subtle bg-elevated/95 p-1.5 shadow-lift backdrop-blur-xl',
  ...
  return (
    <button
      className={cx(
        ...
        danger ? 'text-rose-300 hover:bg-rose-500/10' : 'text-fg hover:bg-white/6'
      )}
      {...p}
    >
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            {title && (
              <div className="sticky top-0 z-10 flex items-center justify-between border-b border-subtle bg-base/95 px-5 py-4 backdrop-blur-xl sm:px-6">
                <h3 className="pr-4 text-lg font-semibold text-fg">{title}</h3>
                <button onClick={onClose} className="rounded-lg p-1.5 text-fg-secondary hover:bg-white/8 hover:text-fg">
                  <X size={18} />
                </button>
              </div>
            )}
...
export function Input({ className, ...p }) {
  return (
    <input
      className={cx(
        'h-11 w-full rounded-xl border border-subtle bg-surface-1 px-3.5 text-sm text-fg placeholder:text-fg-muted outline-none transition focus:border-aurora-violet/55 focus:bg-surface-1 focus:ring-2 focus:ring-aurora-violet/20',
        className
      )}
      {...p}
    />
