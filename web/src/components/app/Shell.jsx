      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 top-full z-50 mt-2 w-[280px] overflow-hidden rounded-2xl border border-subtle bg-menu p-2 shadow-lift backdrop-blur-xl"
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 6, scale: 0.98 }}
            transition={{ duration: 0.16, ease: 'easeOut' }}
            className="absolute right-0 top-full z-50 mt-2 max-h-[70vh] w-[300px] overflow-y-auto rounded-2xl border border-subtle bg-menu p-2 shadow-lift backdrop-blur-xl"
          >
            {groups.map((g) => (
...
function PlanChip({ plan }) {
  if (plan.isAdmin) {
    return (
      <button onClick={() => openPricing()} className="hidden items-center gap-1.5 rounded-xl border border-aurora-mint/40 bg-aurora-mint/10 px-3 py-2 text-xs font-semibold text-ok transition hover:bg-aurora-mint/20 sm:flex">
  ...
  if (plan.planId === 'free') {
    return (
      <button onClick={() => openPricing('pro')} className="hidden items-center gap-1.5 rounded-xl border border-aurora-violet/35 bg-aurora-violet/10 px-3 py-2 text-xs font-bold text-brand transition hover:bg-aurora-violet/20 sm:flex">
        <Zap size={14} /> Upgrade
      </button>
    );
  }
  return (
    <button onClick={() => openPricing()} className="hidden items-center gap-1.5 rounded-xl border border-aurora-mint/30 bg-aurora-mint/10 px-3 py-2 text-xs font-semibold text-ok transition hover:bg-aurora-mint/20 sm:flex">
...
export default function Shell({ active, onPick, title, children }) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <header className={`sticky top-0 z-40 border-b transition-colors duration-300 ${scrolled ? 'border-subtle bg-base/90 backdrop-blur-xl' : 'border-transparent bg-base/40 backdrop-blur-md'}`}>
        ...
        <div className="mx-auto flex h-[var(--shell-topbar)] max-w-7xl items-center gap-3 px-4 sm:px-6">
          ...
          <div className="ml-3 flex shrink-0 items-center gap-2">
            ...
            <Dropdown
              trigger={
                <div className="flex items-center gap-2 rounded-xl border border-subtle bg-surface-1 py-1 pl-1 pr-2 transition hover:bg-surface-1">
            ...
            <button onClick={() => setDrawer(true)} className="grid h-9 w-9 place-items-center rounded-xl border border-subtle text-fg-secondary hover:bg-surface-1 xl:hidden" aria-label="Menu"><Menu size={20} /></button>
          </div>
        </div>
      </header>
      ...
      <AnimatePresence>
        {drawer && [
          <motion.div key="nav-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-scrim backdrop-blur-sm xl:hidden" onClick={() => setDrawer(false)} />,
          <motion.aside
            key="nav-panel"
            initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 320, damping: 32 }}
            className="fixed inset-y-0 right-0 z-50 flex w-[300px] flex-col overflow-y-auto border-l border-subtle bg-base xl:hidden"
          >
            <div className="flex items-center justify-between px-5 py-5">
              <span className="font-display text-[15px] font-extrabold text-fg">Menu</span>
              <button onClick={() => setDrawer(false)} className="rounded-lg p-2 text-fg-secondary hover:bg-surface-1"><X size={18} /></button>
            </div>
