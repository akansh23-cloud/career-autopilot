  ...
  return (
    <div className="space-y-5">
      <Card className="p-5">
        ...
        {s.shortDescription && <p className="mt-4 text-[13px] leading-relaxed text-fg-secondary">{s.shortDescription}</p>}
      </Card>

      {next && (
        <Card className="p-5">
          <SectionTitle>Next best action</SectionTitle>
          <p className="text-[14px] font-semibold text-fg">{next.title || String(next)}</p>
          {next.reason && <p className="mt-1.5 text-[12.5px] text-fg-muted">{next.reason}</p>}
        </Card>
      )}
      ...
      <Card className="p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {[['Must have', m.mustHave], ['Should have', m.shouldHave], ['Later', m.later], ['Excluded from MVP', m.excluded]].map(([label, items]) => (
            <div key={label}>
              <div className="mb-1.5 font-mono text-[10px] uppercase tracking-widest text-fg-muted">{label}</div>
              <ul className="space-y-1">
                {(items || []).map((x, i) => <li key={i} className="text-[13px] text-fg-secondary">• {typeof x === 'string' ? x : x.title || x.name}</li>)}
                {!(items || []).length && <li className="text-[12px] text-fg-muted">—</li>}
              </ul>
            </div>
          ))}
        </div>
      ...
      <Card className="p-5">
        ...
        <div className="flex flex-wrap gap-x-8 gap-y-3 text-[13px] text-fg-secondary">
          <span>{prog.percentDone}% done</span>
          <span className="text-brand">{prog.percentVerified}% verified</span>
          <span>{prog.doneTasks} done / {prog.verifiedTasks} verified / {prog.blockedTasks} blocked of {prog.totalTasks} tasks</span>
        </div>
        <p className="mt-2 text-[11.5px] text-fg-muted">Done = you marked it complete. Verified = the system confirmed evidence. They are tracked separately.</p>
      </Card>
      ...
      <Card className="p-5">
        ...
        {pack.available
          ? <p className="text-[13px] text-fg-secondary">Generated {pack.lastGeneratedAt ? new Date(pack.lastGeneratedAt).toLocaleString() : ''} — {pack.includedFiles?.length || 0} files. <span className="text-fg-muted">A starter skeleton, not a completed project; downloading it does not mark anything done or verified.</span></p>
          : <p className="text-[13px] text-fg-muted">Not generated yet. Use “Preview Starter Pack” or “Download Starter Pack” in the header.</p>}
      </Card>
    </div>
  );
}
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {screens.map((sc) => (
            <button
              ...
              className={`rounded-2xl border p-4 text-left transition ${selected?.id === sc.id ? 'border-aurora-violet/40 bg-aurora-violet/10' : 'border-subtle bg-surface-1 hover:border-strong'}`}
            >
              <div className="mb-3 space-y-1.5 rounded-lg border border-dashed border-subtle bg-sunken p-3">
                <div className="h-2 w-3/4 rounded bg-surface-2" />
                <div className="h-2 w-1/2 rounded bg-surface-1" />
                <div className="mt-2 grid grid-cols-3 gap-1.5">
                  {(sc.components || []).slice(0, 3).map((c, i) => <div key={i} className="h-8 rounded bg-surface-1" />)}
                </div>
              </div>
              <div className="flex items-center justify-between gap-2">
                <span className="truncate text-[13px] font-semibold text-fg">{sc.name}</span>
                <StatusBadge status={sc.status || 'planned'} />
              </div>
              <div className="mt-1 truncate font-mono text-[11px] text-fg-muted">{sc.route} · {sc.userRole}</div>
            </button>
          ))}
        </div>
      </div>
          ...
          {journeys.map((j) => (
            <Card key={j.id} className="p-4">
              <div className="mb-2 flex items-center gap-2">
                <span className="text-[13px] font-semibold text-fg">{j.name}</span>
                <Badge tone="cyan">{j.actor}</Badge>
              </div>
              <div className="flex flex-wrap items-center gap-1.5 text-[12px] text-fg-secondary">
                {(j.steps || []).map((st, i) => (
                  <span key={i} className="flex items-center gap-1.5">
                    <span className="rounded-lg border border-subtle bg-surface-1 px-2 py-1">{typeof st === 'string' ? st : st.label || st.name}</span>
                    {i < j.steps.length - 1 && <span className="text-fg-muted">→</span>}
                  </span>
                ))}
              </div>
            </Card>
