...
function Hint({ icon: Icon, title, children }) {
  ...
  return (
    <div className="rounded-xl border border-subtle bg-surface-1">
      ...
      {open && <div className="border-t border-white/6 px-3.5 py-3 text-[13px] leading-relaxed text-fg-secondary [&_code]:rounded [&_code]:bg-white/8 [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-[12px]">{children}</div>}
    </div>
  );
}
          ...
          <ol className="space-y-2">
            {e.steps.map((s, i) => (
              <li key={i} className="flex gap-2.5 text-[13.5px] leading-relaxed text-fg-secondary">
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-white/8 text-[11px] font-semibold text-fg-secondary">{i + 1}</span>
