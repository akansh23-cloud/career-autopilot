...
function Hint({ icon: Icon, title, children }) {
  ...
  return (
    <div className="rounded-xl border border-white/8 bg-white/[0.02]">
      <button onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-2 px-3.5 py-2.5 text-left text-[13px] font-medium text-slate-200">
        <Icon size={15} className="text-aurora-cyan" /> {title}
        <ChevronRight size={14} className={`ml-auto text-slate-500 transition ${open ? 'rotate-90' : ''}`} />
      </button>
      {open && <div className="border-t border-white/6 px-3.5 py-3 text-[13px] leading-relaxed text-slate-300 [&_code]:rounded [&_code]:bg-white/8 [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-[12px]">{children}</div>}
    </div>
  );
}
    ...
    <>
      {blocks.map((block, i) => {
        if (i % 2 === 1) {
          ...
          return <pre key={i} className="my-2 overflow-x-auto rounded-lg bg-ink-950/70 p-3 text-[12px] leading-relaxed text-slate-200"><code>{body}</code></pre>;
        }
        ...
        return (
          <button key={e.taskId} onClick={() => onPick(i)} title={`${e.no}. ${e.title}`}
            className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg border text-[11px] font-semibold transition ${
              i === activeIdx ? 'border-aurora-violet bg-aurora-violet/20 text-white'
...
export default function GuidedPath({ plan, project, onTaskPatch, onPreviewCode, onOpenBlueprint }) {
  ...
  if (loading) return <div className="flex items-center gap-2 py-16 text-sm text-slate-400"><Spinner /> Building your step-by-step path…</div>;
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">Your build · step by step</p>
            <p className="mt-0.5 text-[15px] font-semibold text-white">{doneCount} of {entries.length} tasks done · {pct}%</p>
          </div>
      ...
      <Card className="p-5 sm:p-6">
        <div className="flex flex-wrap items-center gap-2 text-[12px] text-slate-400">
          ...
          <span className="ml-auto font-mono text-slate-600">Task {e.no} / {String(entries.length).padStart(2, '0')}</span>
        </div>

        <h2 className="mt-2 font-display text-xl font-semibold text-white">{e.title}</h2>
        ...
        <div className="mt-3 rounded-xl border border-aurora-cyan/20 bg-aurora-cyan/[0.05] p-3.5">
          ...
          <p className="mt-1 text-[13.5px] leading-relaxed text-slate-200">{e.why}</p>
        </div>

        {/* files */}
        {e.files.length > 0 && (
          <section className="mt-4">
            <p className="mb-2 flex items-center gap-1.5 text-[13px] font-semibold text-white"><FolderGit2 size={15} className="text-slate-400" /> Open these files</p>
            <div className="space-y-1.5">
              {e.files.map((f) => (
                <div key={f.path} className="flex items-center gap-2 rounded-lg border border-white/8 bg-white/[0.02] px-3 py-2">
                  <FileCode size={14} className="shrink-0 text-slate-500" />
                  <code className="min-w-0 flex-1 truncate text-[12px] text-slate-300">{f.path}</code>
        ...
        {/* do this */}
        <section className="mt-4">
          <p className="mb-2 flex items-center gap-1.5 text-[13px] font-semibold text-white"><ListChecks size={15} className="text-slate-400" /> Do this</p>
          <ol className="space-y-2">
            {e.steps.map((s, i) => (
              <li key={i} className="flex gap-2.5 text-[13.5px] leading-relaxed text-slate-300">
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-white/8 text-[11px] font-semibold text-slate-300">{i + 1}</span>
        ...
        {e.run?.length > 0 && (
          <section className="mt-4">
            <div className="mb-2 flex items-center justify-between">
              <p className="flex items-center gap-1.5 text-[13px] font-semibold text-white"><Terminal size={15} className="text-slate-400" /> Run this</p>
              <CopyButton text={e.run.join('\n')} label="Copy commands" />
            </div>
            <pre className="overflow-x-auto rounded-xl bg-ink-950/70 p-3.5 text-[12.5px] leading-relaxed text-aurora-mint"><code>{e.run.map((c) => `$ ${c}`).join('\n')}</code></pre>
          </section>
        )}

        {/* check yourself */}
        <section className="mt-4">
          <p className="mb-2 flex items-center gap-1.5 text-[13px] font-semibold text-white"><Check size={15} className="text-slate-400" /> Check yourself</p>
          <div className="flex items-center justify-between rounded-lg border border-white/8 bg-white/[0.02] px-3 py-2">
            <code className="text-[12px] text-slate-300">{e.check.command}</code>
          ...
          <ul className="mt-2 space-y-1.5">
            {e.check.criteria.map((c, i) => (
              <li key={i} className="flex items-start gap-2 text-[13px] text-slate-300">
                <Circle size={13} className="mt-0.5 shrink-0 text-slate-600" /> {c}
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="mt-4 space-y-2">
          <p className="text-[13px] font-semibold text-white">Stuck? Open hints one at a time</p>
        ...
        <section className="mt-4 rounded-xl border border-aurora-violet/25 bg-aurora-violet/[0.06] p-4">
          <div className="flex items-center justify-between gap-2">
            <p className="flex items-center gap-1.5 text-[13px] font-semibold text-white"><Bot size={16} className="text-aurora-violet" /> Your AI pair for this task</p>
        ...
        {/* learned */}
        <section className="mt-4 rounded-xl border border-white/8 bg-white/[0.02] p-4">
          <p className="flex items-center gap-1.5 text-[13px] font-semibold text-white"><GraduationCap size={15} className="text-aurora-mint" /> What you’ll have learned</p>
          <ul className="mt-2 space-y-1">
            {e.learn.map((l, i) => <li key={i} className="flex items-start gap-2 text-[13px] leading-relaxed text-slate-300"><Check size={13} className="mt-0.5 shrink-0 text-aurora-mint" /> {l}</li>)}
          </ul>
        </section>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-white/8 pt-4">
          ...
          <span className="text-[11.5px] text-slate-500">Done ≠ Verified — the platform verifies from your repo, tests & deploy.</span>
      ...
      {doneCount === entries.length && (
        <Card className="flex items-center gap-3 border-aurora-mint/30 bg-aurora-mint/[0.06] p-4">
          ...
          <p className="text-[13.5px] text-slate-200">Every task is done. Now push your repo, deploy it, and submit proof — the platform verifies from that evidence, and a short viva confirms you can explain what you built.</p>
        </Card>
      )}
    </div>
  );
