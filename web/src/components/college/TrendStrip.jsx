...
function DeltaPill({ delta, pctChange, higherIsBetter = true, suffix = '' }) {
  ...
  if (delta === 0) {
    return <span className="inline-flex items-center gap-0.5 text-[11px] font-medium text-fg-muted"><Minus size={11} /> no change</span>;
  }
  const up = delta > 0;
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 text-[11px] font-semibold ${good ? 'text-aurora-mint' : 'text-amber-glow'}`}>
      <Icon size={11} />
      {up ? '+' : ''}{delta}{suffix}
      {pctChange != null && <span className="ml-0.5 font-normal text-fg-muted">({up ? '+' : ''}{pctChange}%)</span>}
    </span>
  );
}
...
export default function TrendStrip({ trends }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {stock?.hasBaseline ? (
        <>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {stock.metrics.map((m) => (
              <div key={m.key} className="rounded-xl border border-subtle bg-surface-1 px-3 py-2.5">
                ...
                <div className="mt-1 flex items-baseline gap-2">
                  <span className="font-display text-xl font-bold text-fg">{m.value}</span>
                  {m.previous != null && <span className="text-[11px] text-fg-muted">was {m.previous}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {flowItems.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border border-subtle bg-surface-1 px-3 py-2.5">
          <span className="flex items-center gap-1.5 text-xs text-fg-muted"><Activity size={12} /> Last {flow.windowDays} days vs the {flow.windowDays} before</span>
          {flowItems.map((f) => (
            <span key={f.key} className="flex items-center gap-1.5 text-sm">
              <span className="text-fg-secondary">{f.label}</span>
              <span className="font-semibold text-fg">{f.current}</span>
