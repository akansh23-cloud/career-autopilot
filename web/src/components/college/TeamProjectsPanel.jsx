    ...
    <div className="space-y-5">
      ...
      {(brief.gapPlan || []).length > 0 && (
        <div className="rounded-xl border border-amber-400/20 bg-amber-400/[0.04] p-3">
          <p className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-warn"><AlertTriangle size={13} /> Skill gaps to plan for</p>
    ...
    <>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <Field label="Formation">
            <select
              ...
              className="w-full rounded-xl border border-field-border bg-field px-3 py-2 text-sm text-fg"
            >
              <option value="balanced">Balanced (mixed readiness)</option>
              <option value="similar">Streamed (similar readiness)</option>
            </select>
          </Field>
      ...
      <Modal open={picking} onClose={() => setPicking(false)} width="max-w-3xl" title="Pick students for this team">
        <div className="space-y-4">
          ...
          {roster.error && <p className="text-sm text-warn">{roster.error}</p>}
                ...
                >
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-[11px] text-fg-muted">
                      ...
                      {noSkills && <span className="text-warn/80"> · no declared skills</span>}
                    </span>
                  </span>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Field label="Project type" hint="Blank = best skill match">
            <select
              ...
              className="w-full rounded-xl border border-field-border bg-field px-3 py-2 text-sm text-fg"
            >
              <option value="">Auto — match to team skills</option>
              {catalog.map((a) => <option key={a.id} value={a.id}>{a.title}</option>)}
            </select>
          </Field>
          <Field label="Scope">
            <select
            ...
            <div className="space-y-2">
              {sub.liveUrl && (
                <a href={sub.liveUrl} target="_blank" rel="noreferrer noopener"
                  className="flex items-center gap-2 text-sm text-info underline-offset-2 hover:underline">
          ...
          {(data.profiles || []).map((m) => {
            ...
            return (
              <div key={m.studentId} className="rounded-xl border border-subtle bg-surface-1 p-3">
                ...
                {!m.stillInCohort && <p className="mt-1 text-[11px] text-warn">No longer in your cohort.</p>}
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {!state.loading && state.error && <p className="text-sm text-warn">{state.error}</p>}
                  ...
                  <tr key={p.id} className="border-b border-subtle transition hover:bg-surface-1">
                    ...
                    <td className="px-2 py-2.5">
                      {p.submission?.liveUrl
                        ? <span className="flex items-center gap-1 text-xs text-info"><Globe size={12} /> submitted</span>
