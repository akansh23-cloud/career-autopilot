  ...
  return (
    <Modal open={open} onClose={onClose} width="max-w-3xl" title={initial ? 'Edit drive' : 'New placement drive'}>
      <div className="space-y-4">
        ...
        <div>
          <p className="mb-2 flex items-center gap-1.5 text-sm font-medium text-white"><Filter size={14} /> Eligibility criteria</p>
...
function Funnel({ funnel }) {
  ...
  if (!top) {
    return <p className="py-6 text-center text-sm text-slate-500">No students recorded against this drive yet.</p>;
  }
  return (
    <div className="space-y-2">
      {stages.map((s) => (
        <div key={s.id} className="flex items-center gap-3">
          <span className="w-24 shrink-0 text-xs text-slate-400">{s.label}</span>
          ...
          <span className="w-14 shrink-0 text-right text-sm font-semibold text-white">{s.count}</span>
          <span className="w-12 shrink-0 text-right text-[11px] text-slate-500">{s.conversionFromTop}%</span>
        </div>
      ))}
              ...
              <tbody>
                {list.map((r) => (
                  <tr key={r.id} className="border-t border-white/6 hover:bg-white/[0.02]">
                    ...
                    <td className="px-2 py-2 text-slate-400">{r.branch || '—'} · {r.batch || '—'}</td>
                    ...
                    <td className="px-2 py-2">
                      {showIneligible ? (
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          {r.outcome.ctcLpa != null && <span className="text-xs text-slate-400">{lpa(r.outcome.ctcLpa)}</span>}
                        </span>
                      ...
                      <td className="px-2 py-2 text-right">
                        {r.outcome && (
                          <button
                            ...
                            className="text-xs text-slate-500 hover:text-amber-glow" title="Clear recorded outcome"
                          ><X size={13} /></button>
                        )}
                      </td>
                    )}
      ...
      <Modal open={markOpen} onClose={() => setMarkOpen(false)} title={`Record outcome for ${selected.size} student(s)`}>
        <div className="space-y-3">
          ...
          <p className="text-xs text-slate-500">
            Only <span className="text-slate-300">Accepted</span> counts as a placement in reports. An outstanding
    ...
    <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {drives.map((d) => (
                  <tr key={d.id} className="border-t border-white/6 hover:bg-white/[0.02]">
                    <td className="px-2 py-2">
                      <button onClick={() => setOpenDrive(d.id)} className="text-left text-slate-200 hover:text-white">
                        {d.title}
                        <span className="block text-xs text-slate-500">{[d.company, d.role].filter(Boolean).join(' · ') || '—'}</span>
                      </button>
                    </td>
                    <td className="px-2 py-2"><Badge tone={STATUS_TONES[d.status] || 'default'}>{STATUS_LABELS[d.status] || d.status}</Badge></td>
                    <td className="px-2 py-2 text-right text-slate-300">{d.eligibleCount ?? 0}</td>
                    <td className="px-2 py-2 text-right text-slate-300">{d.participants ?? 0}</td>
                    <td className="px-2 py-2 text-right text-slate-300">{d.offered ?? 0}</td>
                    <td className="px-2 py-2 text-right">{d.placed > 0 ? <Badge tone="mint">{d.placed}</Badge> : <span className="text-slate-600">0</span>}</td>
                    <td className="px-2 py-2 text-right text-slate-300">{lpa(d.medianCtc)}</td>
                    <td className="px-2 py-2 text-right">
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="soft" onClick={() => setOpenDrive(d.id)}><Search size={12} /> Open</Button>
                        <button onClick={() => setConfirmDelete(d)} className="rounded-lg px-2 text-slate-500 hover:text-amber-glow" title="Delete drive"><Trash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <Modal open={Boolean(confirmDelete)} onClose={() => setConfirmDelete(null)} title="Delete this drive?">
        <p className="mb-4 text-sm text-slate-300">
          <span className="text-white">{confirmDelete?.title}</span> and all {confirmDelete?.participants ?? 0} recorded
