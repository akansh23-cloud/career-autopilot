...
function Cell({ value }) {
  ...
  if (value === false) return <X size={15} className="text-fg-muted" />;
...
function ManagePlan({ plan }) {
  ...
  return (
    <div className="mt-5 rounded-2xl border border-subtle bg-surface-1 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <BadgeCheck size={16} className="text-aurora-mint" />
          <span className="text-sm font-semibold text-fg">Manage plan</span>
        </div>
        <div className="flex items-center gap-2">
          {isAdmin
      ...
      {isAdmin ? (
        <p className="mt-3 text-[12px] text-fg-secondary">All features are unlocked on this account — unlimited tailoring, contacts, outreach, tracking, every template and custom uploads. No usage limits apply.</p>
      ) : (
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          {meters.map((m) => {
            ...
            return (
              <div key={m} className="rounded-xl border border-subtle bg-base/55 p-2.5">
                <div className="flex justify-between text-[10px] text-fg-secondary"><span className="capitalize">{METER_LABELS[m]}</span><span>{used}{isUnlimited(max) ? '' : ` / ${max}`}</span></div>
                <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-surface-1"><div className="h-full rounded-full bg-aurora-cta" style={{ width: `${isUnlimited(max) ? 8 : pct}%` }} /></div>
      ...
      {!isAdmin && plan.expiresAt && plan.planId !== 'free' && (
        <p className="mt-2 text-[11px] text-fg-muted">Renews/expires on {new Date(plan.expiresAt).toLocaleDateString()}.</p>
      )}
    </div>
  );
}
...
export default function PricingModal() {
  ...
  return (
    <Modal open={open} onClose={() => setOpen(false)} width="max-w-5xl" title="Plans & pricing">
      {isAdmin && (
        <div className="mb-4 flex items-center gap-2 rounded-xl border border-aurora-mint/30 bg-aurora-mint/10 px-4 py-3 text-sm text-fg">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {PLANS.map((p) => {
          ...
          return (
            <div key={p.id} className={`relative flex flex-col rounded-2xl border p-5 ${p.highlight && !isCurrent && !isAdmin ? 'border-aurora-violet/50 bg-aurora-violet/[0.07] ring-1 ring-aurora-violet/25' : isCurrent ? 'border-aurora-mint/40 bg-aurora-mint/[0.05]' : 'border-subtle bg-surface-1'}`}>
              ...
              <div className="flex items-center gap-2">
                <span className={`grid h-9 w-9 place-items-center rounded-xl bg-surface-1 ring-1 ring-strong ${p.tone === 'violet' ? 'text-aurora-violet' : p.tone === 'amber' ? 'text-amber-glow' : 'text-aurora-cyan'}`}><Icon size={18} /></span>
                <span className="font-display text-lg font-semibold text-fg">{p.name}</span>
              </div>
              <div className="mt-3 flex items-end gap-1">
                <span className="font-display text-3xl font-bold text-fg">{p.price}</span>
                <span className="mb-1 text-xs text-fg-muted">{p.cadence}</span>
              </div>
              <p className="mt-1.5 text-xs leading-snug text-fg-secondary">{p.tagline}</p>

              <ul className="mt-4 flex-1 space-y-2.5 border-t border-subtle pt-4">
                {p.features.map((f) => (
      ...
      {toast && (
        <div className={`mt-5 flex items-center gap-2 rounded-xl border px-4 py-3 text-sm ${toast.type === 'success' ? 'border-aurora-mint/30 bg-aurora-mint/10 text-fg' : 'border-rose-400/30 bg-rose-500/10 text-danger'}`}>
