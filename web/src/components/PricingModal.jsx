...
export default function PricingModal() {
  ...
  return (
    <Modal open={open} onClose={() => setOpen(false)} width="max-w-5xl" title="Plans & pricing">
        ...
        {PLANS.map((p) => {
          ...
          return (
            <div key={p.id} className={`relative flex flex-col rounded-2xl border p-5 ${p.highlight && !isCurrent && !isAdmin ? 'border-aurora-violet/50 bg-aurora-violet/[0.07] ring-1 ring-aurora-violet/25' : isCurrent ? 'border-aurora-mint/40 bg-aurora-mint/[0.05]' : 'border-subtle bg-surface-1'}`}>
              ...
              <div className="flex items-center gap-2">
                <span className={`grid h-9 w-9 place-items-center rounded-xl bg-surface-1 ring-1 ring-white/10 ${p.tone === 'violet' ? 'text-aurora-violet' : p.tone === 'amber' ? 'text-amber-glow' : 'text-aurora-cyan'}`}><Icon size={18} /></span>
      ...
      {toast && (
        <div className={`mt-5 flex items-center gap-2 rounded-xl border px-4 py-3 text-sm ${toast.type === 'success' ? 'border-aurora-mint/30 bg-aurora-mint/10 text-fg' : 'border-rose-400/30 bg-rose-500/10 text-rose-200'}`}>
