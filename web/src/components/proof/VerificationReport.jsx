...
function ResultIcon({ r }) {
  if (r.valid) return <ShieldCheck size={14} className="text-ok" />;
  if (r.tampered || r.revoked) return <ShieldAlert size={14} className="text-danger" />;
  return <ShieldQuestion size={14} className="text-warn" />;
}
