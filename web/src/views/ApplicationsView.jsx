    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
        <SectionCard title="Inputs">
          <Field label="Your name (optional)"><input value={name} onChange={(e) => setName(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-surface-1 px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" placeholder="Jane Smith" /></Field>
          <div className="mt-3"><Field label="Target role (optional)"><input value={role} onChange={(e) => setRole(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-surface-1 px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" placeholder="DevOps Engineer" /></Field></div>
          <div className="mt-3"><Field label="Resume text">
            <textarea value={resumeText} onChange={(e) => setResumeText(e.target.value)} placeholder="Paste your resume (auto-filled if you analyzed one on the Resume page)…" className="h-40 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
          </Field></div>
          <div className="mt-3"><Field label="Job description">
            <textarea value={jd} onChange={(e) => setJd(e.target.value)} placeholder="Paste the full job description…" className="h-40 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
          </Field></div>
          ...
          {!pkg ? (
            ...
            <>
              <SectionCard title="Summary">
                ...
                {!pkg.fabricationSafe && (
                  <ul className="mt-3 space-y-1 border-t border-subtle pt-3">{pkg.fabricationRisks.map((r, i) => <li key={i} className="text-[12px] text-rose-300">• {r.type?.replace(/_/g, ' ')}: {r.detail}</li>)}</ul>
                )}
              </SectionCard>
