...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={() => { navigator.clipboard?.writeText(text); setDone(true); setTimeout(() => setDone(false), 1500); }}
      className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-[11px] text-fg-secondary hover:text-fg">
...
function DocCard({ icon: Icon, title, body }) {
  ...
  return (
    <SectionCard title={title} action={<CopyBtn text={copyText} />}>
      ...
      {isEmail && <div className="mb-2 text-[12px] text-fg-secondary">Subject: <span className="text-fg">{body.subject}</span></div>}
      {isList
        ? <ul className="space-y-1.5">{body.map((s, i) => <li key={i} className="text-[13px] text-fg-secondary">• {s}</li>)}</ul>
        : <pre className="whitespace-pre-wrap font-sans text-[13px] leading-relaxed text-fg-secondary">{isEmail ? body.body : body}</pre>}
    </SectionCard>
  );
}
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
        <SectionCard title="Inputs">
          <Field label="Your name (optional)"><input value={name} onChange={(e) => setName(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" placeholder="Jane Smith" /></Field>
          <div className="mt-3"><Field label="Target role (optional)"><input value={role} onChange={(e) => setRole(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3.5 text-sm text-fg outline-none focus:border-aurora-violet/50" placeholder="DevOps Engineer" /></Field></div>
          <div className="mt-3"><Field label="Resume text">
            <textarea value={resumeText} onChange={(e) => setResumeText(e.target.value)} placeholder="Paste your resume (auto-filled if you analyzed one on the Resume page)…" className="h-40 w-full resize-none rounded-xl border border-field-border bg-field p-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
          </Field></div>
          <div className="mt-3"><Field label="Job description">
            <textarea value={jd} onChange={(e) => setJd(e.target.value)} placeholder="Paste the full job description…" className="h-40 w-full resize-none rounded-xl border border-field-border bg-field p-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
          </Field></div>
        ...
        <div className="space-y-4">
          {!pkg ? (
            <SectionCard><div className="py-10 text-center text-sm text-fg-muted"><FileStack size={28} className="mx-auto mb-2 text-fg-muted" /> Your complete application package appears here.</div></SectionCard>
          ) : (
            <>
              <SectionCard title="Summary">
                ...
                {pkg.usedVerifiedSkills?.length > 0 && (
                  <div className="mt-3"><div className="mb-1 text-[11px] uppercase tracking-wide text-fg-muted">Verified skills used</div>
                ...
                {pkg.missingRequiredSkills?.length > 0 && (
                  <div className="mt-3"><div className="mb-1 text-[11px] uppercase tracking-wide text-fg-muted">JD skills not in your resume (not claimed)</div>
                    <div className="flex flex-wrap gap-1.5">{pkg.missingRequiredSkills.map((s) => <Badge key={s} tone="violet">{s}</Badge>)}</div></div>
                )}
                {!pkg.fabricationSafe && (
                  <ul className="mt-3 space-y-1 border-t border-subtle pt-3">{pkg.fabricationRisks.map((r, i) => <li key={i} className="text-[12px] text-danger">• {r.type?.replace(/_/g, ' ')}: {r.detail}</li>)}</ul>
                )}
              </SectionCard>
