...
function Meter({ icon: Icon, label, value }) {
  ...
  return (
    <div className="flex-1">
      <div className="mb-1 flex items-center justify-between text-[10px] text-fg-muted"><span className="inline-flex items-center gap-1"><Icon size={10} /> {label}</span><span>{value}</span></div>
      <div className="h-1 w-full overflow-hidden rounded-full bg-surface-1"><div className="h-full rounded-full" style={{ width: `${value}%`, background: tone }} /></div>
    </div>
  );
}
...
function InspirationCard({ it, onBuild, onSave, onSource }) {
  return (
    <div className="flex flex-col rounded-2xl border border-subtle bg-surface-1 p-4 transition hover:border-strong">
      ...
      <h3 className="mt-2 text-sm font-semibold text-fg">{it.title}</h3>
      <p className="mt-1 line-clamp-3 text-[13px] text-fg-secondary">{it.problemStatement}</p>
      ...
      {it.tags?.length > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{it.tags.slice(0, 5).map((t) => <span key={t} className="rounded-md bg-surface-1 px-2 py-0.5 text-[10px] text-fg-secondary">{t}</span>)}</div>}
...
export default function InspirationsView({ go }) {
  ...
  return (
    <>
      ...
      {sourceTags.length > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-2 text-[11px] text-fg-muted">
      ...
      {loading ? (
        <SectionCard><div className="flex items-center gap-2 py-8 text-fg-secondary"><Loader2 size={16} className="animate-spin" /> Loading inspirations…</div></SectionCard>
      ) : items.length === 0 ? (
      ...
      {(building || roadmap) && (
        <Modal open onClose={() => { setRoadmap(null); setBuilding(false); }} title={roadmap ? roadmap.title : 'Building roadmap…'} width="max-w-2xl">
          {!roadmap ? <div className="flex items-center gap-2 py-8 text-fg-secondary"><Loader2 size={16} className="animate-spin" /> Generating your build roadmap…</div> : (
            <div className="space-y-4">
              ...
              <p className="text-sm text-fg-secondary">{roadmap.problemStatement}</p>
              {roadmap.architecturePreview && <div><div className="text-[11px] uppercase tracking-wide text-fg-muted">Architecture</div><p className="text-sm text-fg-secondary">{roadmap.architecturePreview}</p></div>}
              {roadmap.techStack?.length > 0 && <div className="flex flex-wrap gap-1.5">{roadmap.techStack.map((t) => <span key={t} className="rounded-md bg-surface-1 px-2 py-0.5 text-[11px] text-fg-secondary">{t}</span>)}</div>}

              <div>
                <div className="mb-1 flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-fg-muted"><ListChecks size={12} /> Milestones</div>
                <ol className="space-y-2">
                  {roadmap.milestones.map((m, i) => (
                    <li key={i} className="rounded-lg border border-subtle bg-surface-1 px-3 py-2 text-sm">
                      <span className="font-medium text-fg">{i + 1}. {m.phase}</span>
                      <div className="text-[13px] text-fg-secondary">{m.detail}</div>
                    </li>
                  ))}
                </ol>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <div className="mb-1 text-[11px] uppercase tracking-wide text-fg-muted">Proof requirements</div>
                  <ul className="space-y-1">{roadmap.proofRequirements.map((p, i) => <li key={i} className="text-[13px] text-fg-secondary">• {p}</li>)}</ul>
                </div>
                <div>
                  <div className="mb-1 flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-fg-muted"><ShieldCheck size={12} /> Verification checklist</div>
                  <ul className="space-y-1">{roadmap.verificationChecklist.map((c, i) => <li key={i} className="text-[13px] text-fg-secondary">{c.required ? '◆' : '◇'} {c.item}</li>)}</ul>
                </div>
              </div>

              {roadmap.resumeBullets?.length > 0 && (
                <div><div className="mb-1 text-[11px] uppercase tracking-wide text-fg-muted">Expected resume bullets</div>
                  <ul className="space-y-1">{roadmap.resumeBullets.map((b, i) => <li key={i} className="text-[13px] text-fg-secondary">• {b}</li>)}</ul></div>
              )}
              {roadmap.interviewTalkingPoints?.length > 0 && (
                <div><div className="mb-1 text-[11px] uppercase tracking-wide text-fg-muted">Interview talking points</div>
                  <ul className="space-y-1">{roadmap.interviewTalkingPoints.map((b, i) => <li key={i} className="text-[13px] text-fg-secondary">• {b}</li>)}</ul></div>
              )}
              ...
              <p className="text-[11px] text-fg-muted">When you've built it, submit the project with GitHub/live proof in Skills &amp; XP to earn verified XP.</p>
            </div>
          )}
        </Modal>
      )}
