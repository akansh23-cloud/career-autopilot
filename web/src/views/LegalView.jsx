...
function Sections({ sections }) {
  return (
    <div className="space-y-7">
      {sections.map((s) => (
        <section key={s.h}>
          <h2 className="font-display text-[15px] font-semibold text-white">{s.h}</h2>
          {s.p.map((para, i) => (
            <p key={i} className="mt-2 text-[13.5px] leading-relaxed text-slate-300">{para}</p>
          ))}
        </section>
      ))}
    </div>
    ...
    <div className="space-y-4">
      {rows.map(([label, value, note]) => (
        <div key={label} className="rounded-xl border border-white/8 bg-white/[0.03] p-4">
          ...
          {note && <p className="mt-1 text-[12px] leading-relaxed text-slate-400">{note}</p>}
        </div>
      ))}
      <p className="text-[12px] leading-relaxed text-slate-500">
        In-app support tickets (Help widget) reach the same team and are the fastest route for account-specific issues. Consent version currently in force: <span className="font-mono text-slate-400">{meta?.consentVersion || '—'}</span>.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <main className="relative mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <button onClick={onBack} className="mb-6 inline-flex items-center gap-1.5 text-[13px] text-slate-400 transition hover:text-white">
          <ArrowLeft size={14} /> Back to Career Autopilot
        </button>

        <h1 className="font-display text-2xl font-semibold text-white">Legal & policies</h1>
          ...
          {TABS.map((t) => (
            <button key={t.id} onClick={() => pick(t.id)}
              className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[13px] transition ${
                tab === t.id ? 'border-white/25 bg-white/[0.07] text-white' : 'border-white/8 bg-white/[0.02] text-slate-300 hover:border-white/20'
              }`}>
        ...
        <div className="mt-7 rounded-2xl border border-white/8 bg-white/[0.02] p-6 sm:p-8">
