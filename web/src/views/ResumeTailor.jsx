    ...
    <div className="mt-6 space-y-4">
      <SectionCard title="Tailor resume for a job" eyebrow="Job Fit">
        ...
        <textarea value={jd} onChange={(e) => setJd(e.target.value)}
          ...
          className="mt-3 h-40 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-4 text-sm leading-relaxed text-fg outline-none placeholder:text-fg-muted focus:border-aurora-violet/50" />
        ...
        <>
          ...
          <SectionCard title="Tailored resume (review before use)" action={<Button size="sm" variant="soft" onClick={() => saveVersion('job')}><Save size={14} /> Save job version</Button>}>
            <textarea readOnly value={result.tailoredResume?.text || ''}
              className="h-72 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-4 text-xs leading-relaxed text-fg outline-none" />
          ...
          <div className="space-y-2">
            {versions.map((v) => (
              <div key={v.id} className="flex items-center justify-between gap-3 rounded-xl border border-subtle bg-surface-1 px-3 py-2">
                ...
                <button onClick={() => removeVersion(v.id)} className="rounded-md p-1.5 text-fg-muted hover:text-rose-300" title="Delete version"><Trash2 size={15} /></button>
              </div>
            ))}
          </div>
        )}
