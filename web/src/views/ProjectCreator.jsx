...
function Accordion({ title, icon: Icon, defaultOpen = false, badge, children }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-subtle bg-surface-1">
      ...
      {open && <div className="border-t border-subtle px-4 py-3 text-[13px] leading-relaxed text-fg-secondary">{children}</div>}
    </div>
  );
}
...
function List({ items, tone = 'cyan' }) {
  ...
  if (!list.length) return <p className="text-[13px] text-fg-muted">Not available.</p>;
  return (
    ...
    <div className="space-y-2">
      {rows.map((r) => (
        <div key={r.key} className="flex items-center gap-3 text-[12px]">
          <span className="w-40 shrink-0 text-fg-secondary">{labels[r.key] || r.key}</span>
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-white/8">
            <div className="h-full rounded-full bg-aurora-cta" style={{ width: `${Math.round((r.score / (r.weight || r.max || 20)) * 100)}%` }} />
          </div>
          <span className="w-12 text-right font-mono text-fg-secondary">{r.score}/{r.weight || r.max}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-subtle bg-surface-1 p-4">
      <div className="mb-2 flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-fg">Deterministic checks</p>
        <Badge tone={result.requiredOk ? 'mint' : 'amber'}>{result.passed}/{result.total} passed</Badge>
        <span className="ml-auto text-[11px] text-fg-muted">Objective · not AI-generated</span>
      </div>
    ...
    <div className="flex flex-wrap gap-2">
      {CREATOR_STEPS.map((s, i) => {
        ...
        return (
          ...
          >
            <span className={`grid h-6 w-6 place-items-center rounded-lg text-[11px] ${on ? 'bg-aurora-cta text-ink-950' : 'bg-white/8 text-fg-secondary'}`}>{i + 1}</span>
            <Icon size={14} className={on ? 'text-aurora-cyan' : ''} /> {s.label}
            {locked && <Lock size={11} className="text-fg-muted" />}
          </button>
        );
      })}
    </div>
      ...
      <SectionCard title="Where do you want to start?" action={<Badge tone={recLeft === Infinity ? 'mint' : recLeft > 0 ? 'cyan' : 'amber'}>{recLeft === Infinity ? 'Unlimited' : `${recLeft} left`}</Badge>}>
        <div className="flex flex-wrap gap-2">
          {START_SOURCES.map((s) => (
            <button key={s.id} onClick={() => { setSource(s.id); if (s.id === 'github' || s.id === 'producthunt') { setTab('marketplace'); loadTrends(); } }}
              className={`rounded-full border px-3 py-1.5 text-xs font-medium transition ${source === s.id ? 'border-aurora-cyan/50 bg-aurora-cyan/15 text-[#B9EFFA]' : 'border-white/12 bg-surface-1 text-fg-secondary hover:bg-white/8'}`}>
              {s.label}
            </button>
          ))}
        </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Field label="Difficulty">
            <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-base px-3 text-sm text-fg">
              {DIFFICULTIES.map((d) => <option key={d}>{d}</option>)}
            </select>
          </Field>
          <Field label="Duration">
            <select value={duration} onChange={(e) => setDuration(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-base px-3 text-sm text-fg">
              {DURATIONS.map((d) => <option key={d}>{d}</option>)}
            </select>
          </Field>
          <Field label="Preferred type">
            <select value={preferredType} onChange={(e) => setPreferredType(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-base px-3 text-sm text-fg">
          ...
          <div className="mt-3">
            <Field label="Describe your idea" hint="A sentence or two — we’ll validate and shape it into a buildable product.">
              <textarea value={customIdea} onChange={(e) => setCustomIdea(e.target.value)} rows={3}
                className="w-full rounded-xl border border-subtle bg-base px-3 py-2.5 text-sm text-fg placeholder:text-fg-muted" placeholder="e.g. A tool that turns lecture recordings into searchable notes…" />
            </Field>
          </div>
        )}
        ...
        recs && recs.length ? (
          <div className="space-y-5">
            {CATEGORY_ORDER.filter((c) => byCategory[c]?.length).map((cat) => (
              <div key={cat}>
                <p className="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-widest text-fg-muted"><Star size={12} className="text-amber-glow" /> {cat}</p>
    ...
    <div className="flex flex-col rounded-2xl border border-subtle bg-surface-1 p-4 transition hover:border-strong">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-2 font-medium text-fg">{rec.title}</p>
          <p className="mt-1 text-[13px] leading-relaxed text-fg-secondary">{rec.summary}</p>
        </div>
        <ScoreRing score={fit.score} label="Fit" />
      </div>

      <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px]">
        <div className="rounded-xl border border-subtle bg-surface-1 p-2"><p className="font-display text-base font-semibold text-fg">{rec.difficulty}</p><p className="text-fg-muted">Level</p></div>
        <div className="rounded-xl border border-subtle bg-surface-1 p-2"><p className="font-display text-base font-semibold text-fg">{rec.estimatedDuration}</p><p className="text-fg-muted">Duration</p></div>
        <div className="rounded-xl border border-subtle bg-surface-1 p-2"><p className="font-display text-base font-semibold text-fg">{rec.startupPotential}</p><p className="text-fg-muted">Startup</p></div>
      </div>
      ...
      <div className="mt-3 space-y-2">
        <Accordion title="Why recommended + fit breakdown" icon={Target}>
          ...
          {rec.sourceSignals?.length > 0 && <p className="mt-3 text-[12px] text-fg-muted">Signals: {rec.sourceSignals.join(', ')}.</p>}
        </Accordion>
        <Accordion title="Skills & impact" icon={Award}>
          <p className="text-fg-secondary">Covers gaps: <span className="text-fg">{(rec.missingSkillsCovered || []).join(', ') || '—'}</span></p>
          <p className="mt-1 text-fg-secondary">Still missing: <span className="text-fg">{(rec.stillMissingSkills || []).join(', ') || 'none'}</span></p>
          <p className="mt-2 text-fg-secondary">Target users: <span className="text-fg">{rec.targetUsers}</span></p>
          ...
          {rec.expectedProofOutputs?.length > 0 && <div className="mt-2"><span className="text-fg-secondary">Expected proof: </span>{rec.expectedProofOutputs.map((o, i) => <Badge key={i} tone="mint">{o}</Badge>)}</div>}
        </Accordion>
      </div>
    ...
    <SectionCard title="Idea marketplace" action={<Lightbulb size={16} className="text-amber-glow" />}>
      <div className="mb-4 flex flex-wrap gap-2">
        {MARKETPLACE_TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`rounded-full border px-3 py-1.5 text-xs font-medium transition ${tab === t ? 'border-aurora-violet/40 bg-aurora-violet/15 text-fg' : 'border-white/12 bg-surface-1 text-fg-secondary hover:bg-white/8'}`}>{t}</button>
        ))}
      </div>
      ...
      {tab === 'Published Projects' && (
        published.length ? (
          <div className="grid gap-3 md:grid-cols-2">
            {published.map((p) => (
              <div key={p.id} className="rounded-2xl border border-subtle bg-surface-1 p-4">
                <div className="flex items-center justify-between"><p className="font-medium text-fg">{p.title}</p><StatusBadge status={creatorStatus(p).status} /></div>
                <p className="mt-1 text-[13px] text-fg-secondary">{p.summary || p.problemStatement}</p>
      ...
      {tab === 'Trending from GitHub' && (
        trends?.github?.length ? (
          <div className="grid gap-3 md:grid-cols-2">
            {trends.github.map((g) => (
              <div key={g.name} className="rounded-2xl border border-subtle bg-surface-1 p-4">
                <div className="flex items-center justify-between"><a href={g.url} target="_blank" rel="noreferrer" className="font-medium text-fg hover:text-aurora-cyan">{g.name}</a><Badge tone="amber"><Star size={11} /> {g.stars}</Badge></div>
                <p className="mt-1 text-[13px] text-fg-secondary">{g.description || 'No description.'}</p>
...
function IdeaCard({ idea, onBuild, busy }) {
  return (
    <div className="rounded-2xl border border-subtle bg-surface-1 p-4">
      ...
      <p className="mt-2 font-medium text-fg">{idea.title}</p>
      <p className="mt-1 text-[13px] leading-relaxed text-fg-secondary">{idea.problem}</p>
      <div className="mt-2 flex flex-wrap gap-1.5">{idea.skills.slice(0, 5).map((s, i) => <Badge key={i} tone="cyan">{s}</Badge>)}</div>
      <div className="mt-3 flex items-center justify-between">
        <span className="text-[12px] text-fg-muted">{idea.difficulty}</span>
        ...
        {loading ? <Loading msg="Generating validation suggestions…" /> : !data ? (
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-fg">AI estimated score: {data.total}/100 <span className="text-[12px] font-normal text-fg-muted">(suggestion)</span></p>
                <p className="text-[13px] text-fg-secondary">{data.total >= 75 ? 'AI thinks this is strong — verify with the checks above and real user signal.' : data.total >= 55 ? 'AI sees promise — tighten differentiation and users.' : 'AI suggests sharpening the problem and proof.'}</p>
              </div>
            </div>
            {r?.genericWarning && (
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Accordion title="Problem & users" icon={Users} defaultOpen><p className="mb-2"><span className="text-fg-secondary">Severity: </span>{r.problemSeverity}</p><p className="mb-2"><span className="text-fg-secondary">Users: </span>{r.targetUsers}</p><p className="mb-1 text-fg-secondary">Pain points:</p><List items={r.userPainPoints} /></Accordion>
              <Accordion title="Alternatives & differentiation" icon={Scale}><p className="mb-1 text-fg-secondary">Existing alternatives:</p><List items={r.existingAlternatives} /><p className="mb-1 mt-2 text-fg-secondary">Why they’re weak:</p><List items={r.whyAlternativesWeak} tone="amber" /></Accordion>
              <Accordion title="Feasibility & monetization" icon={TrendingUp}><p className="mb-2"><span className="text-fg-secondary">MVP feasibility: </span>{r.mvpFeasibility}</p><p className="mb-2"><span className="text-fg-secondary">Build difficulty: </span>{r.buildDifficulty}</p><p className="mb-1 text-fg-secondary">Monetization:</p><List items={r.monetization} tone="mint" /></Accordion>
              <Accordion title="Risks & assumptions" icon={AlertTriangle}><p className="mb-1 text-fg-secondary">Risks:</p><List items={r.risks} tone="amber" /><p className="mb-1 mt-2 text-fg-secondary">Assumptions:</p><List items={r.assumptions} /></Accordion>
              <Accordion title="Validation questions" icon={ShieldCheck}><List items={r.validationQuestions} /></Accordion>
              <Accordion title="First 10 users & metrics" icon={Rocket}><p className="mb-1 text-fg-secondary">First 10 users:</p><List items={r.firstTenUsersStrategy} tone="mint" /><p className="mb-1 mt-2 text-fg-secondary">Success metrics:</p><List items={r.successMetrics} /></Accordion>
            </div>
          </div>
        )}
      </SectionCard>
    ...
    <div className="space-y-4">
      <SectionCard title="Product blueprint" action={<Button size="sm" variant={bp ? 'soft' : 'primary'} onClick={run} disabled={loading}>{loading ? 'Building…' : bp ? <><RefreshCw size={14} /> Re-run</> : <><Wand2 size={14} /> Generate blueprint</>}</Button>}>
        {loading ? <Loading msg="Building product blueprint…" /> : !bp ? (
          ...
          <div className="space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-xl border border-subtle bg-surface-1 p-3"><p className="text-[11px] uppercase tracking-widest text-fg-muted">Vision</p><p className="mt-1 text-[13px] text-fg">{toText(bp.productVision)}</p></div>
              <div className="rounded-xl border border-subtle bg-surface-1 p-3"><p className="text-[11px] uppercase tracking-widest text-fg-muted">Positioning</p><p className="mt-1 text-[13px] text-fg">{toText(bp.positioning)}</p></div>
            </div>

            <div>
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-widest text-fg-muted">System architecture</p>
              ...
              <ArchitectureStudioPanel
                ...
                height={380}
              />
              <p className="mt-2 text-[12px] text-fg-secondary">{toText(bp.systemArchitecture)}</p>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Accordion title="Personas & journeys" icon={Users}><p className="mb-1 text-fg-secondary">Personas:</p><List items={bp.personas} /><p className="mb-1 mt-2 text-fg-secondary">Journeys:</p><List items={bp.userJourneys} /></Accordion>
              <Accordion title="Tech stack & integrations" icon={Boxes}><div className="flex flex-wrap gap-1.5">{toList(bp.techStack).map((s, i) => <Badge key={i} tone="cyan">{s}</Badge>)}</div><p className="mt-2 text-fg-secondary">Integrations:</p><List items={bp.integrations} /></Accordion>
              ...
              <Accordion title="UI screens & folder structure" icon={Boxes}><List items={bp.uiScreens} /><pre className="mt-2 overflow-x-auto rounded-lg border border-subtle bg-base p-3 text-[12px] text-fg-secondary">{toText(bp.folderStructure)}</pre></Accordion>
              <Accordion title="Security & non-functional" icon={ShieldCheck}><p className="mb-1 text-fg-secondary">Security:</p><List items={bp.securityRequirements} tone="amber" /><p className="mb-1 mt-2 text-fg-secondary">Non-functional:</p><List items={bp.nonFunctional} /></Accordion>
              <Accordion title="Deployment, testing & analytics" icon={Rocket}><p className="mb-1 text-fg-secondary">Deployment:</p><List items={bp.deploymentArchitecture} /><p className="mb-1 mt-2 text-fg-secondary">Testing:</p><List items={bp.testingStrategy} /><p className="mb-1 mt-2 text-fg-secondary">Analytics:</p><List items={bp.analytics} /></Accordion>
    ...
    <div className="space-y-4">
      <SectionCard title="Beginner-friendly build roadmap" action={
        <div className="flex items-center gap-2">
          <select value={level} onChange={(e) => setLevel(e.target.value)} className="h-9 rounded-lg border border-subtle bg-base px-2 text-[13px] text-fg">{DIFFICULTIES.map((d) => <option key={d}>{d}</option>)}</select>
        ...
        {loading ? <Loading msg="Creating mentor-level roadmap…" /> : !tasks.length ? (
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-subtle bg-surface-1 p-3">
              <div className="flex items-center justify-between text-xs text-fg-secondary"><span>Roadmap progress</span><span>{progress}%</span></div>
              ...
              <p className="mt-2 text-[12px] text-fg-muted">Completing tasks raises your proof score and skill XP. GitHub/live proof (next step) gives stronger XP and unlocks verification.</p>
            </div>
            {Object.entries(grouped).map(([milestone, mtasks]) => (
              <div key={milestone}>
                <p className="mb-2 text-[11px] font-semibold uppercase tracking-widest text-fg-muted">{milestone}</p>
                <div className="space-y-2">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <button onClick={() => setOpen((v) => !v)} className="text-left">
            <p className={`text-sm font-medium ${done ? 'text-fg-secondary line-through' : 'text-fg'}`}>{t.title}</p>
            {t.whyItMatters && <p className="mt-0.5 text-[12px] text-fg-muted">Why: {t.whyItMatters}</p>}
          </button>
          {open && (
            <div className="mt-2 space-y-2 border-t border-subtle pt-2 text-[12px] text-fg-secondary">
              {t.detailedSteps?.length > 0 && <div><p className="text-fg-secondary">Steps:</p><List items={t.detailedSteps} /></div>}
        ...
        <button onClick={() => setOpen((v) => !v)} className="text-fg-muted">{open ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</button>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4">
      <SectionCard title="Verification" action={<StatusBadge status={st.status} />}>
        ...
        {st.reasons?.length > 0 && (
          <div className="mt-3 rounded-xl border border-subtle bg-surface-1 p-3">
            <p className="text-[12px] font-medium text-fg-secondary">To reach {st.next}:</p>
      ...
      <SectionCard title="Proof of work">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <p className="mb-1.5 text-[12px] font-medium text-fg-secondary">GitHub repository</p>
          ...
          <div>
            <p className="mb-1.5 text-[12px] font-medium text-fg-secondary">Live demo</p>
      ...
      <SectionCard title="IP Readiness" action={<Button size="sm" variant="soft" onClick={() => setIpOpen(true)}><Scale size={14} /> Open IP studio</Button>}>
        <p className="text-[13px] text-fg-secondary">Optional educational IP-readiness guidance and draft preparation — patent readiness score, novelty points, prior-art keywords and a provisional spec outline.</p>
...
function Check2({ label, checked, onChange }) {
  return (
    <label className="flex cursor-pointer items-start gap-2 text-[13px] text-fg-secondary">
    ...
    <div className="flex flex-wrap items-center gap-1.5">
      {PATENT_STAGES.map((s, i) => (
        <button key={s.id} onClick={() => onSet(s.id)}
          className={`rounded-full border px-2.5 py-1 text-[11px] font-medium transition ${i <= idx ? 'border-aurora-mint/40 bg-aurora-mint/12 text-[#BDF5DC]' : 'border-white/12 bg-surface-1 text-fg-secondary hover:bg-white/8'}`}>
  ...
  return (
    <div className="mt-4 rounded-2xl border border-subtle bg-surface-1 p-4">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <p className="text-sm font-semibold text-fg">Patent registration</p>
        <Badge tone={eligible ? 'mint' : 'amber'}>{rep?.verdict || (eligible ? 'Eligible' : 'Not yet eligible')}</Badge>
        <span className="ml-auto font-mono text-[11px] text-fg-muted">criteria-gated · not legal advice</span>
      </div>
      ...
      <div className="mb-3 space-y-1.5">
        {rep?.criteria && Object.keys(rep.criteria).map((k) => (
          <div key={k} className="flex items-center gap-3 text-[12px]">
            <span className="w-52 shrink-0 text-fg-secondary">{({ technicalCharacter: 'Technical character', novelty: 'Novelty', inventiveStep: 'Inventive step', industrialApplicability: 'Industrial use', enablement: 'Enablement' })[k]}</span>
            ...
            <span className="w-10 text-right font-mono text-fg-secondary">{rep.criteria[k]}</span>
          </div>
        ))}
      </div>

      {/* attestations that drive novelty scoring */}
      <div className="mb-3 space-y-1.5 rounded-xl border border-subtle bg-surface-1 p-3">
        <p className="text-[12px] font-medium text-fg-secondary">Confirm to score novelty accurately:</p>
      ...
      {!eligible ? (
        ...
        <div className="space-y-3">
          {!dossier?.stageId ? (
            ...
            <>
              <div>
                <p className="mb-1.5 text-[12px] font-medium text-fg-secondary">Filing pipeline</p>
                <PatentPipeline stageId={dossier.stageId} onSet={setStage} />
              </div>

              <Accordion title="1 · Prior-art search plan" icon={Scale} defaultOpen>
                <p className="mb-1 text-fg-secondary">Classification hint: <span className="text-fg">{d?.priorArt?.classification}</span></p>
                <p className="mb-1 text-fg-secondary">Search queries:</p>
              ...
              <Accordion title="2 · Invention disclosure" icon={Lightbulb}>
                <p className="mb-1 text-fg-secondary">Field:</p><p className="mb-2">{d?.disclosure?.fieldOfInvention}</p>
                <p className="mb-1 text-fg-secondary">Summary of invention:</p><p className="mb-2">{d?.disclosure?.summaryOfInvention}</p>
                <p className="mb-1 text-fg-secondary">Technical advantages:</p><List items={d?.disclosure?.advantages} tone="mint" />
              </Accordion>

              <Accordion title="3 · Provisional specification + claims" icon={FileText}>
                <p className="mb-1 text-fg-secondary">Spec outline:</p><List items={d?.provisionalSpecOutline} />
              ...
              <Accordion title="4 · Filing route & jurisdiction" icon={Globe}>
                <p className="mb-1 text-fg-secondary">Routes:</p>
                <List items={(d?.filing?.routes || []).map((r) => `${r.name} — ${r.when} (${r.note})`)} />
                <p className="mb-1 mt-2 text-fg-secondary">Jurisdictions:</p>
                {(d?.filing?.jurisdictions || []).map((j) => (
                  <div key={j.id} className="mb-2 rounded-lg border border-subtle bg-surface-1 p-2.5 text-[12px]">
                    <p className="font-medium text-fg">{j.name}</p>
                    <p className="text-fg-secondary">Forms: {j.forms.join(', ')}</p>
              ...
              <Accordion title="5 · Record your filing" icon={BadgeCheck}>
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field label="Route"><select value={route} onChange={(e) => setRoute(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-base px-3 text-sm text-fg">{(d?.filing?.routes || []).map((r) => <option key={r.id}>{r.name}</option>)}</select></Field>
                  <Field label="Jurisdiction"><select value={jur} onChange={(e) => setJur(e.target.value)} className="h-11 w-full rounded-xl border border-subtle bg-base px-3 text-sm text-fg">{(d?.filing?.jurisdictions || []).map((j) => <option key={j.id}>{j.name}</option>)}</select></Field>
                </div>
      ...
      {!isPremium ? (
        ...
        <div className="mt-4 space-y-3">
          <div className="flex items-center gap-4">
            ...
            <div><p className="text-sm font-medium text-fg">{rep.patentReadinessScore}/100</p><Badge tone={rep.classification === 'Patent Review Recommended' ? 'amber' : 'violet'}>{rep.classification}</Badge></div>
          </div>
          <Accordion title="Invention summary & problem" icon={Lightbulb} defaultOpen><p className="mb-2">{rep.inventionSummary}</p><p className="mb-1 text-fg-secondary">Technical problem:</p><p className="mb-2">{rep.technicalProblem}</p><p className="mb-1 text-fg-secondary">Technical solution:</p><p>{rep.technicalSolution}</p></Accordion>
          <Accordion title="Novelty & inventive step" icon={Sparkles}><p className="mb-1 text-fg-secondary">Novelty points:</p><List items={rep.noveltyPoints} /><p className="mt-2"><span className="text-fg-secondary">Inventive step: </span>{rep.inventiveStepHypothesis}</p><p className="mt-1"><span className="text-fg-secondary">Industrial use: </span>{rep.industrialUse}</p></Accordion>
          <Accordion title="Prior-art research" icon={Scale}><p className="mb-1 text-fg-secondary">Keywords:</p><div className="flex flex-wrap gap-1.5">{(rep.priorArtKeywords || []).map((k, i) => <Badge key={i} tone="cyan">{k}</Badge>)}</div><p className="mb-1 mt-2 text-fg-secondary">Comparables to research:</p><List items={rep.comparableSolutions} /></Accordion>
      ...
      <SectionCard title="Publish to Sandbox marketplace" action={<StatusBadge status={st.status} />}>
        {!canPublish ? (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm font-medium text-fg">{published ? 'Published to the Sandbox marketplace.' : 'Ready to publish.'}</p>
              <p className="text-[13px] text-fg-secondary">Published projects appear in the recruiter console and your Career Proof Profile.</p>
            </div>
...
function CollabModal({ open, onClose, project, isPaid, go }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Collaboration post draft" width="max-w-xl">
      <div className="space-y-3 text-[13px] text-fg-secondary">
        <div className="rounded-xl border border-subtle bg-surface-1 p-3">
          <p className="font-medium text-fg">{draft.hook}</p>
          <p className="mt-2">{draft.problemStatement}</p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <div><p className="text-fg-secondary">Skills involved:</p><div className="mt-1 flex flex-wrap gap-1.5">{draft.skillsInvolved.map((s, i) => <Badge key={i} tone="cyan">{s}</Badge>)}</div></div>
          <div><p className="text-fg-secondary">Skills needed:</p><div className="mt-1 flex flex-wrap gap-1.5">{draft.skillsNeeded.length ? draft.skillsNeeded.map((s, i) => <Badge key={i} tone="amber">{s}</Badge>) : <span className="text-fg-muted">—</span>}</div></div>
          <div><p className="text-fg-secondary">Roles needed:</p><p className="text-fg">{draft.rolesNeeded.join(', ')}</p></div>
          <div><p className="text-fg-secondary">Stage:</p><p className="text-fg">{draft.projectStage} · {draft.estimatedDuration} · {draft.weeklyTime}</p></div>
        </div>
        ...
        {!isPaid && !posted && <p className="text-[12px] text-fg-muted">Publishing collaboration posts requires Pro.</p>}
      </div>
    </Modal>
  );
}
