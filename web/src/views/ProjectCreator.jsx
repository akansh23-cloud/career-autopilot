      ...
      <SectionCard title="Where do you want to start?" action={<Badge tone={recLeft === Infinity ? 'mint' : recLeft > 0 ? 'cyan' : 'amber'}>{recLeft === Infinity ? 'Unlimited' : `${recLeft} left`}</Badge>}>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Field label="Difficulty">
            <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
              {DIFFICULTIES.map((d) => <option key={d}>{d}</option>)}
            </select>
          </Field>
          <Field label="Duration">
            <select value={duration} onChange={(e) => setDuration(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
              {DURATIONS.map((d) => <option key={d}>{d}</option>)}
            </select>
          </Field>
          <Field label="Preferred type">
            <select value={preferredType} onChange={(e) => setPreferredType(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
          ...
          <div className="mt-3">
            <Field label="Describe your idea" hint="A sentence or two — we’ll validate and shape it into a buildable product.">
              <textarea value={customIdea} onChange={(e) => setCustomIdea(e.target.value)} rows={3}
                className="w-full rounded-xl border border-field-border bg-field px-3 py-2.5 text-sm text-fg placeholder:text-fg-muted" placeholder="e.g. A tool that turns lecture recordings into searchable notes…" />
            </Field>
          </div>
        )}
...
function ValidateStep({ selected, isPremium, setStep, pickProject }) {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="mb-3 flex gap-2 rounded-xl border border-aurora-violet/25 bg-aurora-violet/10 px-3 py-2 text-[12px] text-brand">
    ...
    <div className="space-y-4">
      <SectionCard title="Beginner-friendly build roadmap" action={
        <div className="flex items-center gap-2">
          <select value={level} onChange={(e) => setLevel(e.target.value)} className="h-9 rounded-lg border border-field-border bg-field px-2 text-[13px] text-fg">{DIFFICULTIES.map((d) => <option key={d}>{d}</option>)}</select>
        ...
        {loading ? <Loading msg="Creating mentor-level roadmap…" /> : !tasks.length ? (
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-subtle bg-surface-1 p-3">
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-surface-1"><div className="h-full rounded-full bg-aurora-cta" style={{ width: `${progress}%` }} /></div>
      ...
      <div className="mb-3 space-y-1.5">
        {rep?.criteria && Object.keys(rep.criteria).map((k) => (
          <div key={k} className="flex items-center gap-3 text-[12px]">
            ...
            <div className="h-2 flex-1 overflow-hidden rounded-full bg-surface-1"><div className={`h-full rounded-full ${rep.criteria[k] >= 55 ? 'bg-aurora-mint' : 'bg-amber-glow'}`} style={{ width: `${rep.criteria[k]}%` }} /></div>
            ...
            <>
              ...
              <Accordion title="5 · Record your filing" icon={BadgeCheck}>
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field label="Route"><select value={route} onChange={(e) => setRoute(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">{(d?.filing?.routes || []).map((r) => <option key={r.id}>{r.name}</option>)}</select></Field>
                  <Field label="Jurisdiction"><select value={jur} onChange={(e) => setJur(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">{(d?.filing?.jurisdictions || []).map((j) => <option key={j.id}>{j.name}</option>)}</select></Field>
                </div>
