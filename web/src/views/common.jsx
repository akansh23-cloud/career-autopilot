    ...
    >
      ...
      <div className="relative flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 flex-1">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-aurora-violet/30 bg-aurora-violet/10 px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.2em] text-brand">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-2.5">
              ...
              {secondary.map((s) => (
                <button
                  key={s.label}
                  onClick={s.onClick}
                  className="inline-flex items-center gap-1.5 rounded-xl border border-subtle bg-surface-1 px-4 py-3 text-[13px] text-fg-secondary transition hover:border-aurora-violet/40 hover:text-fg"
                >
...
export function PageIntro({ title, sub, action, eyebrow }) {
  return (
    ...
    >
      ...
      <div className="mt-7 h-px w-full bg-gradient-to-r from-aurora-violet/35 via-surface-2 to-transparent" />
    </motion.header>
  );
}
