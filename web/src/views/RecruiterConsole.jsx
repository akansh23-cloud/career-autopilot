    ...
    <div className="space-y-1">
      {Object.entries(parts).map(([k, v]) => (
        <div key={k} className="flex items-center gap-2 text-[11px]">
          ...
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-surface-1"><div className="h-full rounded-full bg-aurora-cta" style={{ width: `${Math.min(100, v * 3)}%` }} /></div>
    ...
    <>
      ...
      <SectionCard title="Search candidates" action={<Filter size={16} className="text-aurora-cyan" />}>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <select value={role} onChange={(e) => setRole(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
        ...
        {(campusFacets.colleges.length > 0 || campusFacets.branches.length > 0) && (
          <div className="mt-3 border-t border-subtle pt-3">
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
              <select value={fCollege} onChange={(e) => setFCollege(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
                <option value="">Any college</option>
                {campusFacets.colleges.map(([id, name]) => <option key={id} value={id}>{name}</option>)}
              </select>
              <select value={fBranch} onChange={(e) => setFBranch(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
                <option value="">Any specialisation</option>
                {campusFacets.branches.map((b) => <option key={b} value={b}>{b}</option>)}
              </select>
              <select value={fBatch} onChange={(e) => setFBatch(e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-subtle pt-3">
          ...
          {savedSearches.length === 0
            ...
            : savedSearches.map((s) => (
              <span key={s.id} className="inline-flex items-center gap-1.5 rounded-full border border-subtle bg-surface-1 px-2.5 py-1 text-xs text-fg-secondary">
                <button onClick={() => applySaved(s)} className="hover:text-fg">{s.name}</button>
                <button onClick={() => removeSaved(s.id)} className="text-fg-muted hover:text-danger" aria-label="Remove saved search">×</button>
              </span>
            ))}
        </div>
      </SectionCard>
