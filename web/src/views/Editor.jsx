        ...
        <div className="mb-2.5 flex flex-wrap gap-1.5">
          {sections.map((s) => (
            <button key={s} onClick={() => { setActive(active === s ? null : s); setBullet(''); setShowNew(false); }}
              className={`flex items-center gap-1 rounded-lg border px-2.5 py-1 text-[11px] transition ${active === s ? 'border-aurora-violet/50 bg-aurora-violet/12 text-fg' : 'border-subtle text-fg-secondary hover:border-white/22 hover:text-fg'}`}>
      ...
      {active && (
        <div className="mb-2.5 flex gap-2">
          <input autoFocus value={bullet} onChange={(e) => setBullet(e.target.value)}
            ...
            className="h-8 flex-1 rounded-lg border border-subtle bg-surface-1 px-3 text-xs text-fg outline-none placeholder:text-fg-muted focus:border-aurora-violet/50" />
          ...
          <button onClick={() => setActive(null)} className="h-8 rounded-lg bg-surface-1 px-3 text-[11px] text-fg-secondary hover:bg-white/8">✕</button>
        </div>
      )}
      {showNew ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setShowNew(false); setCustom(''); }} className="h-8 rounded-lg bg-surface-1 px-3 text-[11px] text-fg-secondary hover:bg-white/8">Cancel</button>
          </div>
        </div>
      ) : (
        <button onClick={() => { setShowNew(true); setActive(null); }} className="flex items-center gap-1.5 rounded-lg border border-dashed border-white/12 px-3 py-1.5 text-[11px] text-fg-muted transition hover:border-aurora-cyan/40 hover:text-aurora-cyan">
  ...
  if (!allowed) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-strong bg-surface-1 p-5 text-center">
        <div className="grid h-11 w-11 place-items-center rounded-xl bg-surface-1 text-aurora-violet ring-1 ring-white/10"><Lock size={18} /></div>
      ...
      <div className="relative flex h-[176px] items-center justify-center border-b border-subtle bg-[#0d1018] p-2">
        {preview ? (
          <>
            ...
            {status === 'analyzing' && <div className="absolute inset-0 grid place-items-center bg-black/55"><span className="flex items-center gap-1.5 text-[11px] text-fg"><Loader2 size={13} className="animate-spin" /> Analysing layout…</span></div>}
          </>
        ) : (
        ...
        <div className="mt-auto flex flex-wrap gap-2 pt-1">
          {status === 'done' ? (
            <>
              ...
              <button onClick={reAnalyze} title="Re-analyse template" className="rounded-lg border border-white/12 px-2.5 py-1.5 text-[11px] text-fg-secondary hover:bg-surface-2"><RefreshCw size={12} /></button>
              <button onClick={clear} className="rounded-lg border border-white/12 px-2.5 py-1.5 text-[11px] text-fg-secondary hover:text-red-400">Remove</button>
            </>
          ) : preview ? (
            <button onClick={clear} className="flex-1 rounded-lg border border-white/12 py-1.5 text-[11px] text-fg-secondary hover:text-red-400">Remove upload</button>
          ) : (
            <button onClick={() => inputRef.current?.click()} className="flex-1 rounded-lg border border-white/12 bg-surface-1 py-1.5 text-[11px] font-medium text-fg hover:bg-surface-2">Choose file</button>
          )}
        </div>
      </div>
    </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <textarea
              ...
              className="h-52 w-full resize-y rounded-xl border border-subtle bg-surface-1 p-3.5 text-sm text-fg outline-none placeholder:text-fg-muted focus:border-aurora-violet/50"
            />
          ...
          <SectionCard title="Target job description">
            <textarea value={jd} onChange={(e) => setJd(e.target.value)}
              ...
              className="h-40 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-3.5 text-sm text-fg outline-none placeholder:text-fg-muted focus:border-aurora-violet/50" />
          </SectionCard>
