...
function UserCard({ u, onOpen, i }) {
  return (
    ...
    >
      ...
      <div className="mt-3 flex items-center justify-between border-t border-subtle pt-2.5 text-[11px] text-fg-muted">
  ...
  return (
    <>
      <motion.div
        ...
        className="fixed inset-0 z-40 bg-scrim backdrop-blur-sm" onClick={onClose}
      />
      <motion.aside
                ...
                <div className="grid gap-2.5 sm:grid-cols-2">
                  <label className="text-xs text-fg-secondary">Account type
                    <select value={vForm.accountType} onChange={(e) => setVForm((f) => ({ ...f, accountType: e.target.value }))}
                      className="mt-1 w-full rounded-lg border border-field-border bg-field px-2.5 py-2 text-sm text-fg">
                  ...
                  <label className="text-xs text-fg-secondary">Organization id (recruiter)
                    <input value={vForm.organizationId} onChange={(e) => setVForm((f) => ({ ...f, organizationId: e.target.value }))}
                      className="mt-1 w-full rounded-lg border border-field-border bg-field px-2.5 py-2 text-sm text-fg" placeholder="org-…" />
                  </label>
                  <label className="text-xs text-fg-secondary sm:col-span-2">College id (placement cell)
                    <input value={vForm.collegeId} onChange={(e) => setVForm((f) => ({ ...f, collegeId: e.target.value }))}
                      className="mt-1 w-full rounded-lg border border-field-border bg-field px-2.5 py-2 text-sm text-fg" placeholder="college-…" />
                  </label>
                </div>
              ...
              {data.projects?.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1.5">
                    {data.projects.map((p) => (
                      <div key={p.id || p.title} className="flex items-center justify-between rounded-lg border border-subtle bg-surface-1 px-3 py-2 text-sm">
              ...
              <div>
                <Field label="Admin notes" hint="Internal only — never shown to the user or recruiters.">
                  <textarea
                    ...
                    className="w-full rounded-xl border border-field-border bg-field px-3 py-2.5 text-sm text-fg placeholder:text-fg-muted outline-none transition focus:border-aurora-violet/50 focus:ring-2 focus:ring-aurora-violet/20"
                  />
                </Field>
...
function PendingVerificationPanel({ requests, loading, error, busyKey, onApprove, onReject, onRefresh, onOpen }) {
  ...
  return (
    <SectionCard className="mb-6">
      ...
      {error ? (
        <p className="rounded-xl border border-rose-400/20 bg-rose-400/5 px-3 py-2 text-sm text-danger">{error}</p>
      ) : loading ? (
...
export default function AdminUsers() {
  ...
  return (
    <div>
      ...
      {/* Job Discovery OS — operator-triggered ingestion. Populates the same
          canonical store the autonomous workers write to. */}
      <div className="mb-6">
        <AdminJobIngestPanel />
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2.5">
        ...
        <select
          ...
          className="h-11 rounded-xl border border-field-border bg-field px-3 text-sm text-fg outline-none focus:border-aurora-violet/50"
        >
          {SORTS.map((s) => <option key={s.id} value={s.id} className="bg-menu">{s.label}</option>)}
        </select>
        <button
            ...
            <SectionCard className="mb-5">
              <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Field label="Experience level">
                  <select value={filters.experienceLevel} onChange={(e) => setFilter('experienceLevel', e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg outline-none focus:border-aurora-violet/50">
                    <option value="" className="bg-menu">Any level</option>
                    {EXPERIENCE_LEVELS.map((l) => <option key={l} value={l} className="bg-menu">{l}</option>)}
                  </select>
                </Field>
                <Field label="User type">
                  <select value={filters.userType} onChange={(e) => setFilter('userType', e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg outline-none focus:border-aurora-violet/50">
                    {USER_TYPES.map((t) => <option key={t.id} value={t.id} className="bg-menu">{t.label}</option>)}
                  </select>
                </Field>
                <Field label="Project completion">
                  <select value={filters.projectStatus} onChange={(e) => setFilter('projectStatus', e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg outline-none focus:border-aurora-violet/50">
                    {PROJECT_STATUS.map((t) => <option key={t.id} value={t.id} className="bg-menu">{t.label}</option>)}
                  </select>
                </Field>
                <Field label="Activity">
                  <select value={filters.activity} onChange={(e) => setFilter('activity', e.target.value)} className="h-11 w-full rounded-xl border border-field-border bg-field px-3 text-sm text-fg outline-none focus:border-aurora-violet/50">
                    {ACTIVITY.map((t) => <option key={t.id} value={t.id} className="bg-menu">{t.label}</option>)}
                  </select>
                </Field>
