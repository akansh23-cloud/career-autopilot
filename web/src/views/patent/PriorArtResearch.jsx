          ...
          {withPlan.length > 0 && (
            <SectionCard title={`Plan generated (${withPlan.length})`}>
              <div className="space-y-2">{withPlan.map((i) => (
                <button key={i.id} onClick={() => go?.('patentworkspace', { ideaId: i.id })} className="flex w-full items-center justify-between gap-3 rounded-lg border border-white/8 bg-white/[0.02] px-3 py-2 text-left transition hover:border-white/20">
                  <span className="min-w-0"><span className="block truncate text-[13px] text-slate-200">{i.title}</span><span className="text-[11px] text-slate-500">{(i.priorArtSearchPlan.keywords || []).slice(0, 5).join(', ')}</span></span>
                  <span className="flex shrink-0 items-center gap-2"><Badge tone={riskTone(i.score?.riskLevel)}>{i.score?.riskLevel || 'Med'} risk</Badge><ArrowRight size={14} className="text-slate-500" /></span>
                </button>
              ))}</div>
            </SectionCard>
          )}
