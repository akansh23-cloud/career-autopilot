    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="relative">
          <select value={idea.status} onChange={(e) => setStatus(e.target.value)} className="h-9 cursor-pointer appearance-none rounded-lg border border-subtle bg-surface-1 pl-3 pr-8 text-xs text-fg outline-none">
        ...
        <div className="space-y-4">
          ...
          <SectionCard title={`Prior-art records (${priorArt.length})`}>
            <div className="grid gap-2 sm:grid-cols-2">
              ...
              <div className="relative"><select value={paForm.riskLevel} onChange={(e) => setPaForm((p) => ({ ...p, riskLevel: e.target.value }))} className="h-11 w-full cursor-pointer appearance-none rounded-xl border border-subtle bg-surface-1 px-3.5 pr-10 text-sm text-fg outline-none"><option>Low</option><option>Medium</option><option>High</option></select><ChevronDown size={16} className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-fg-muted" /></div>
            </div>
            <textarea value={paForm.summary} onChange={(e) => setPaForm((p) => ({ ...p, summary: e.target.value }))} placeholder="Summary / overlap / differences" className="mt-2 h-16 w-full resize-none rounded-xl border border-subtle bg-surface-1 p-3 text-sm text-fg outline-none" />
            ...
            <div className="mt-3 space-y-2">
              {priorArt.map((r) => (
                <div key={r.id} className="flex items-start justify-between gap-2 rounded-lg border border-subtle bg-surface-1 px-3 py-2">
                  ...
                  <button onClick={() => delPriorArt(r.id)} className="shrink-0 rounded-md p-1.5 text-fg-muted hover:text-rose-300"><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
          </SectionCard>
