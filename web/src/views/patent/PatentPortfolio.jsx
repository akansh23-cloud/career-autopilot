    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load()} placeholder="Search title / problem / tag…" className="h-9 w-full rounded-lg border border-subtle bg-surface-1 pl-9 pr-3 text-xs text-fg outline-none" />
        </div>
        {[['Domain', domain, setDomain, DOMAINS], ['Status', status, setStatus, STATUS_ORDER]].map(([label, val, set, opts]) => (
          <div key={label} className="relative">
            <select value={val} onChange={(e) => set(e.target.value)} className="h-9 cursor-pointer appearance-none rounded-lg border border-subtle bg-surface-1 pl-3 pr-8 text-xs text-fg outline-none">
        ...
        <div className="relative"><select value={minScore} onChange={(e) => setMinScore(e.target.value)} className="h-9 cursor-pointer appearance-none rounded-lg border border-subtle bg-surface-1 pl-3 pr-8 text-xs text-fg outline-none"><option value="">Score: any</option><option value="70">70+</option><option value="55">55+</option><option value="35">35+</option></select><ChevronDown size={13} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-fg-muted" /></div>
      </div>
            ...
            {ideas.filter((i) => !i.archived).map((idea) => (
              <div key={idea.id} className="flex items-center justify-between gap-3 rounded-xl border border-subtle bg-surface-1 px-3 py-2.5">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <div className="relative"><select value={idea.status} onChange={(e) => changeStatus(idea.id, e.target.value)} className="h-8 cursor-pointer appearance-none rounded-lg border border-subtle bg-surface-1 pl-2.5 pr-7 text-[11px] text-fg outline-none">{STATUS_ORDER.map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}</select><ChevronDown size={12} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-fg-muted" /></div>
                  <button onClick={() => setConfirm(idea)} className="rounded-md p-1.5 text-fg-muted hover:text-rose-300" title={LOCKED_STATUSES.includes(idea.status) ? 'Archive' : 'Delete'}>{LOCKED_STATUSES.includes(idea.status) ? <Archive size={15} /> : <Trash2 size={15} />}</button>
