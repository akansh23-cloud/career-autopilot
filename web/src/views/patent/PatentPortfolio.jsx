    ...
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load()} placeholder="Search title / problem / tag…" className="h-9 w-full rounded-lg border border-white/10 bg-white/[0.03] pl-9 pr-3 text-xs text-slate-100 outline-none" />
        </div>
        {[['Domain', domain, setDomain, DOMAINS], ['Status', status, setStatus, STATUS_ORDER]].map(([label, val, set, opts]) => (
          <div key={label} className="relative">
            <select value={val} onChange={(e) => set(e.target.value)} className="h-9 cursor-pointer appearance-none rounded-lg border border-white/10 bg-white/[0.03] pl-3 pr-8 text-xs text-slate-100 outline-none">
        ...
        <div className="relative"><select value={minScore} onChange={(e) => setMinScore(e.target.value)} className="h-9 cursor-pointer appearance-none rounded-lg border border-white/10 bg-white/[0.03] pl-3 pr-8 text-xs text-slate-100 outline-none"><option value="">Score: any</option><option value="70">70+</option><option value="55">55+</option><option value="35">35+</option></select><ChevronDown size={13} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-500" /></div>
      </div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-3">
          {grouped.filter((g) => g.items.length || ['raw_idea', 'shortlisted', 'refining'].includes(g.status)).map((g) => (
            <div key={g.status} className="w-64 shrink-0">
              <div className="mb-2 flex items-center justify-between px-1"><span className="text-[12px] font-medium text-slate-300">{STATUS_LABELS[g.status]}</span><Badge tone="default">{g.items.length}</Badge></div>
              <div className="space-y-2">
                {g.items.map((idea) => (
                  <button key={idea.id} onClick={() => go?.('patentworkspace', { ideaId: idea.id })} className="block w-full rounded-xl border border-white/8 bg-white/[0.02] p-3 text-left transition hover:border-white/20">
                    <div className="text-[13px] font-medium text-white line-clamp-2">{idea.title}</div>
        ...
        <SectionCard>
          <div className="space-y-2">
            {ideas.filter((i) => !i.archived).map((idea) => (
              <div key={idea.id} className="flex items-center justify-between gap-3 rounded-xl border border-white/8 bg-white/[0.02] px-3 py-2.5">
                <button onClick={() => go?.('patentworkspace', { ideaId: idea.id })} className="min-w-0 flex-1 text-left">
                  <div className="truncate text-sm text-slate-200">{idea.title}</div>
                  <div className="mt-0.5 flex items-center gap-1.5"><ScorePill score={idea.score?.overall || 0} />{idea.domain && <span className="text-[11px] text-slate-500">{idea.domain}</span>}</div>
                </button>
                <div className="flex shrink-0 items-center gap-2">
                  <div className="relative"><select value={idea.status} onChange={(e) => changeStatus(idea.id, e.target.value)} className="h-8 cursor-pointer appearance-none rounded-lg border border-white/10 bg-white/[0.03] pl-2.5 pr-7 text-[11px] text-slate-100 outline-none">{STATUS_ORDER.map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}</select><ChevronDown size={12} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-slate-500" /></div>
                  <button onClick={() => setConfirm(idea)} className="rounded-md p-1.5 text-slate-500 hover:text-rose-300" title={LOCKED_STATUSES.includes(idea.status) ? 'Archive' : 'Delete'}>{LOCKED_STATUSES.includes(idea.status) ? <Archive size={15} /> : <Trash2 size={15} />}</button>
                  <button onClick={() => go?.('patentworkspace', { ideaId: idea.id })} className="rounded-md p-1.5 text-slate-500 hover:text-white"><ArrowRight size={15} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      {confirm && (
        <Modal open onClose={() => setConfirm(null)} title={LOCKED_STATUSES.includes(confirm.status) ? 'Archive invention?' : 'Delete invention?'}>
          <div className="space-y-3">
            <p className="flex items-start gap-2 text-sm text-slate-300"><AlertTriangle size={16} className="mt-0.5 shrink-0 text-amber-glow" /> {LOCKED_STATUSES.includes(confirm.status) ? `"${confirm.title}" is ${STATUS_LABELS[confirm.status]} and cannot be permanently deleted — it will be archived.` : `Permanently delete "${confirm.title}"? This cannot be undone.`}</p>
