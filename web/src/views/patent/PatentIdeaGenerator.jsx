  ...
  return (
    <Field label={label}>
      <div className="relative">
        <select value={value} onChange={onChange} className="h-11 w-full cursor-pointer appearance-none rounded-xl border border-field-border bg-field px-3.5 pr-10 text-sm text-fg outline-none focus:border-aurora-violet/50">
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-col gap-2 rounded-xl border border-aurora-violet/30 bg-aurora-violet/[0.07] p-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-[13px] text-fg">Looking for the full experience? Start in the <span className="font-semibold text-fg">Innovation OS</span> — source-backed discovery, build blueprints, prior-art plans, and IP review.</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
        <SectionCard title="Inputs">
          ...
          <div className="mt-3"><Field label="Problem statement">
            <textarea value={form.problem} onChange={f('problem')} placeholder="What real problem should the invention solve?" className="h-20 w-full resize-none rounded-xl border border-field-border bg-field p-3 text-sm text-fg outline-none focus:border-aurora-violet/50" />
          </Field></div>
        ...
        <div className="space-y-3">
          {meta.why && <div className="rounded-lg border border-aurora-violet/25 bg-aurora-violet/5 px-3 py-2 text-[12px] text-fg-secondary"><span className="text-aurora-violet">Why these: </span>{meta.why}{meta.source === 'deterministic' && ' (standard template version — AI generation is not enabled on this account)'}</div>}
          {!ideas.length && !busy && (
            <SectionCard><div className="py-10 text-center text-sm text-fg-muted"><Wand2 size={26} className="mx-auto mb-2 text-fg-muted" /> Generated ideas appear here. Save the strong ones to your portfolio.</div></SectionCard>
          )}
          {busy && <SectionCard><div className="flex items-center gap-2 py-8 text-fg-secondary"><Loader2 size={16} className="animate-spin" /> Composing inventions…</div></SectionCard>}
          {ideas.map((idea, i) => (
            <div key={idea.id || i} className="rounded-2xl border border-subtle bg-surface-1 p-4">
              <div className="flex items-start justify-between gap-2">
                <h3 className="text-sm font-semibold text-fg">{idea.title}</h3>
              ...
              <p className="mt-1.5 text-[13px] text-fg-secondary"><span className="text-fg-muted">Problem: </span>{idea.problem}</p>
              <p className="mt-1 text-[13px] text-fg-secondary"><span className="text-fg-muted">Mechanism: </span>{idea.technicalMechanism}</p>
              <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
                ...
                {idea.targetUser && <span className="rounded-md bg-surface-1 px-2 py-0.5 text-fg-secondary">{idea.targetUser}</span>}
                {(idea.tags || []).slice(0, 4).map((t) => <span key={t} className="rounded-md bg-surface-1 px-2 py-0.5 text-fg-secondary">{t}</span>)}
              </div>
