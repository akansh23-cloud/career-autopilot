          ...
          {disclosures.length > 0 && (
            <SectionCard title={`Drafted disclosures (${disclosures.length})`}>
              <div className="space-y-2">{disclosures.map((d) => (
                <button key={d.id} onClick={() => go?.('patentworkspace', { ideaId: d.ideaId })} className="flex w-full items-center justify-between gap-3 rounded-lg border border-subtle bg-surface-1 px-3 py-2 text-left transition hover:border-strong">
                  <span className="min-w-0 truncate text-[13px] text-fg">{titleFor(d.ideaId)}</span>
                  <span className="flex shrink-0 items-center gap-2"><Badge tone="cyan">v{d.version}</Badge><ArrowRight size={14} className="text-fg-muted" /></span>
                </button>
              ))}</div>
            </SectionCard>
          )}
