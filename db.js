...
/* ---- Innovation & Patent Intelligence OS ----
   Source-backed problem discovery + buildable project synthesis. */
const problemSignalSchema = new mongoose.Schema(
  {
    ...
    rawTextHash: { type: String, default: '', index: true },
    tags: { type: [String], default: [] },
    engagement: { type: mongoose.Schema.Types.Mixed, default: {} },
    sourceCreatedAt: { type: String, default: '' },
    lastActivityAt: { type: String, default: '' },
    extractedPainPoints: { type: [mongoose.Schema.Types.Mixed], default: [] },
    domain: { type: String, default: '', index: true },
...
problemSignalSchema.index({ userId: 1, rawTextHash: 1 }, { unique: true });
...
const problemClusterSchema = new mongoose.Schema(
  {
    ...
    sourceCitations: { type: [mongoose.Schema.Types.Mixed], default: [] },
    keywords: { type: [String], default: [] },
    evidenceStrengthScore: { type: Number, default: 0 },
    severityScore: { type: Number, default: 0 },
    trendScore: { type: Number, default: 0 },
    buildFeasibilityScore: { type: Number, default: 0 },
    portfolioValueScore: { type: Number, default: 0 },
    researchPotentialScore: { type: Number, default: 0 },
    patentPotentialScore: { type: Number, default: 0 },
    recommendedRoute: { type: String, default: 'portfolio' },
    confidence: { type: String, default: 'low' },
    sourceBacked: { type: Boolean, default: false },
    dedupeFingerprint: { type: String, default: '', index: true },
  },
  { timestamps: true }
);
problemClusterSchema.index({ userId: 1, dedupeFingerprint: 1 }, { unique: true });

const generatedInnovationProjectSchema = new mongoose.Schema(
  {
    userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, index: true },
    email: { type: String, lowercase: true, trim: true, index: true },
    clusterId: { type: String, default: '', index: true },
    title: { type: String, default: '' },
    payload: { type: mongoose.Schema.Types.Mixed, default: {} },
    fingerprint: { type: String, default: '', index: true },
    convertedProjectId: { type: String, default: '' },
    convertedPatentIdeaId: { type: String, default: '' },
  },
  { timestamps: true }
);
generatedInnovationProjectSchema.index({ userId: 1, fingerprint: 1 }, { unique: true });
...
export const ProblemSignal = mongoose.models.ProblemSignal || mongoose.model('ProblemSignal', problemSignalSchema);
export const ProblemCluster = mongoose.models.ProblemCluster || mongoose.model('ProblemCluster', problemClusterSchema);
export const GeneratedInnovationProject = mongoose.models.GeneratedInnovationProject || mongoose.model('GeneratedInnovationProject', generatedInnovationProjectSchema);
...
/* ============================================================
   PROBLEM INTELLIGENCE OS  (source-backed innovation projects)
   ============================================================ */
function publicProblemSignal(d) {
  if (!d) return null;
  const o = d.toObject ? d.toObject() : d;
  return { ...o, id: String(o._id || o.id || o.rawTextHash || '') };
}
function publicProblemCluster(d) {
  if (!d) return null;
  const o = d.toObject ? d.toObject() : d;
  return { ...o, id: String(o._id || o.id || o.dedupeFingerprint || '') };
}
function publicInnovationProject(d) {
  if (!d) return null;
  const o = d.toObject ? d.toObject() : d;
  return { ...o.payload, id: String(o._id || o.id || ''), projectRecordId: String(o._id || o.id || ''), clusterId: o.clusterId || o.payload?.clusterId || '', convertedProjectId: o.convertedProjectId || '', convertedPatentIdeaId: o.convertedPatentIdeaId || '' };
}

export async function saveProblemIntelligenceRun({ userId, email, signals = [], clusters = [] }) {
  if (!URI) return { ok: false, reason: 'db_disabled', signals, clusters };
  try {
    await connectDB();
    const uid = await resolveUserId({ userId, email });
    if (!uid) return { ok: false, reason: 'user_not_found', signals, clusters };
    const savedSignals = [];
    const rawToId = new Map();
    for (const s of signals.slice(0, 80)) {
      const doc = {
        userId: uid, email: cleanEmail(email), source: s.source || 'manual', sourceId: String(s.sourceId || '').slice(0, 200),
        sourceUrl: String(s.sourceUrl || '').slice(0, 1000), title: String(s.title || '').slice(0, 300),
        contentSummary: String(s.contentSummary || '').slice(0, 2000), rawTextHash: String(s.rawTextHash || '').slice(0, 100),
        tags: Array.isArray(s.tags) ? s.tags.slice(0, 20) : [], engagement: s.engagement || {},
        sourceCreatedAt: String(s.sourceCreatedAt || ''), lastActivityAt: String(s.lastActivityAt || ''),
        extractedPainPoints: Array.isArray(s.extractedPainPoints) ? s.extractedPainPoints.slice(0, 5) : [],
        domain: String(s.domain || '').slice(0, 80), technology: String(s.technology || '').slice(0, 80), targetUser: String(s.targetUser || '').slice(0, 120),
      };
      const saved = await ProblemSignal.findOneAndUpdate(
        { userId: uid, rawTextHash: doc.rawTextHash || `${doc.source}:${doc.sourceId}` },
        { $set: doc },
        { upsert: true, new: true, setDefaultsOnInsert: true }
      );
      rawToId.set(s.rawTextHash, String(saved._id));
      savedSignals.push(publicProblemSignal(saved));
    }
    const savedClusters = [];
    for (const c of clusters.slice(0, 20)) {
      const doc = {
        userId: uid, email: cleanEmail(email), title: String(c.title || '').slice(0, 300), summary: String(c.summary || '').slice(0, 1000),
        domain: String(c.domain || '').slice(0, 80), technology: String(c.technology || '').slice(0, 80), targetUser: String(c.targetUser || '').slice(0, 120),
        signalIds: Array.isArray(c.signals) ? c.signals.map((s) => rawToId.get(s.rawTextHash) || s.sourceId).filter(Boolean).slice(0, 50) : (c.signalIds || []).slice(0, 50),
        signals: Array.isArray(c.signals) ? c.signals.slice(0, 10) : [], sourceCitations: Array.isArray(c.sourceCitations) ? c.sourceCitations.slice(0, 10) : [],
        keywords: Array.isArray(c.keywords) ? c.keywords.slice(0, 20) : [],
        evidenceStrengthScore: Number(c.evidenceStrengthScore || 0), severityScore: Number(c.severityScore || 0), trendScore: Number(c.trendScore || 0),
        buildFeasibilityScore: Number(c.buildFeasibilityScore || 0), portfolioValueScore: Number(c.portfolioValueScore || 0), researchPotentialScore: Number(c.researchPotentialScore || 0), patentPotentialScore: Number(c.patentPotentialScore || 0),
        recommendedRoute: String(c.recommendedRoute || 'portfolio'), confidence: String(c.confidence || 'low'), sourceBacked: !!c.sourceBacked,
        dedupeFingerprint: String(c.dedupeFingerprint || c.id || '').slice(0, 80),
      };
      const saved = await ProblemCluster.findOneAndUpdate(
        { userId: uid, dedupeFingerprint: doc.dedupeFingerprint || doc.title },
        { $set: doc },
        { upsert: true, new: true, setDefaultsOnInsert: true }
      );
      savedClusters.push(publicProblemCluster(saved));
    }
    return { ok: true, signals: savedSignals, clusters: savedClusters };
  } catch (err) {
    console.error('[db] saveProblemIntelligenceRun failed:', err.message);
    return { ok: false, reason: 'db_error', error: err.message, signals, clusters };
  }
}

export async function listProblemClusters({ userId, email, limit = 100 }) {
  if (!URI) return [];
  try {
    await connectDB();
    const uid = await resolveUserId({ userId, email });
    if (!uid) return [];
    const docs = await ProblemCluster.find({ userId: uid }).sort({ updatedAt: -1 }).limit(Math.min(Number(limit) || 100, 200)).lean();
    return docs.map(publicProblemCluster);
  } catch (err) { console.error('[db] listProblemClusters failed:', err.message); return []; }
}

export async function getProblemCluster({ userId, email, id }) {
  if (!URI) return null;
  try {
    await connectDB();
    const uid = await resolveUserId({ userId, email });
    if (!uid) return null;
    const q = isObjectId(id) ? { _id: id, userId: uid } : { dedupeFingerprint: String(id || ''), userId: uid };
    const doc = await ProblemCluster.findOne(q).lean();
    return publicProblemCluster(doc);
  } catch (err) { console.error('[db] getProblemCluster failed:', err.message); return null; }
}

export async function saveGeneratedInnovationProject({ userId, email, project = {} }) {
  if (!URI) return { ok: false, reason: 'db_disabled', project };
  try {
    await connectDB();
    const uid = await resolveUserId({ userId, email });
    if (!uid) return { ok: false, reason: 'user_not_found', project };
    const fp = String(project.fingerprint || '').slice(0, 100) || String(project.title || '').toLowerCase().slice(0, 100);
    const saved = await GeneratedInnovationProject.findOneAndUpdate(
      { userId: uid, fingerprint: fp },
      { $set: { userId: uid, email: cleanEmail(email), clusterId: String(project.clusterId || ''), title: String(project.title || '').slice(0, 300), payload: project, fingerprint: fp } },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    );
    return { ok: true, project: publicInnovationProject(saved), duplicateSafe: true };
  } catch (err) { console.error('[db] saveGeneratedInnovationProject failed:', err.message); return { ok: false, reason: 'db_error', error: err.message, project }; }
}

export async function getGeneratedInnovationProject({ userId, email, id }) {
  if (!URI) return null;
  try {
    await connectDB();
    const uid = await resolveUserId({ userId, email });
    if (!uid) return null;
    const doc = await GeneratedInnovationProject.findOne({ _id: id, userId: uid }).lean();
    return publicInnovationProject(doc);
  } catch (err) { console.error('[db] getGeneratedInnovationProject failed:', err.message); return null; }
}

export async function updateGeneratedInnovationProjectLinks({ userId, email, id, convertedProjectId = '', convertedPatentIdeaId = '' }) {
  if (!URI || !isObjectId(id)) return { ok: false, reason: 'db_disabled_or_invalid_id' };
  try {
    await connectDB();
    const uid = await resolveUserId({ userId, email });
    if (!uid) return { ok: false, reason: 'user_not_found' };
    const patch = {};
    if (convertedProjectId) patch.convertedProjectId = String(convertedProjectId);
    if (convertedPatentIdeaId) patch.convertedPatentIdeaId = String(convertedPatentIdeaId);
    await GeneratedInnovationProject.updateOne({ _id: id, userId: uid }, { $set: patch });
    return { ok: true };
  } catch (err) { console.error('[db] updateGeneratedInnovationProjectLinks failed:', err.message); return { ok: false, reason: 'db_error', error: err.message }; }
}
